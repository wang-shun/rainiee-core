...
@ResourceAnnotation
public class BondExecutor extends AbstractOrderExecutor {

	public BondExecutor(ResourceProvider resourceProvider) {
		super(resourceProvider);
	}

	@Override
	public Class<? extends Resource> getIdentifiedType() {
		return BondExecutor.class;
	}
	...
	protected void doConfirm(SQLConnection connection, int orderId,
		...
		try {
			...
			if (wlAcount.F06.compareTo(t6513.F03) < 0) {
				throw new LogicalException("用户往来账户余额不足");
			}
			// 用户风险保证金账户
			...
			fxAcount.F06 = fxAcount.F06.add(t6513.F03);
			t6102 = new T6102();
			...
			String content = configureProvider.format(LetterVariable.FXBZJCZ, envionment);
			sendLetter(connection, t6513.F02, "风险保证金充值成功", content);
			...
			if ("1".equals(isUseYtx))
			{
			    ...
			    int type = smsUtils.getTempleId(MsgVariavle.FXBZJCZ.getDescription());
			    sendMsg(connection, t6110.F04, envionment.get("amount"), type);
			}
			else
			{
				String msgContent = configureProvider.format(MsgVariavle.FXBZJCZ, envionment);
