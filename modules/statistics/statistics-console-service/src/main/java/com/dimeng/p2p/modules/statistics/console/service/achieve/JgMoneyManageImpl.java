...
public class JgMoneyManageImpl extends AbstractStatisticsService implements JgMoneyManage
{
    
    public JgMoneyManageImpl(ServiceResource serviceResource)
    {
        ...
        throws Throwable
    {
        ...
        sql.append("(SELECT IFNULL(SUM(F07),0)  FROM S62.T6252 WHERE T6252.F03 = Y_t6110.F01 AND T6252.F09 ='WH') loanAmount, ");
        ...
        sql.append("FROM S61.T6110 Y_t6110,S61.T6161 Y_t6161  WHERE Y_t6161.F01=Y_t6110.F01 AND Y_t6110.F06 = 'FZRR' AND Y_t6110.F13 = 'F' AND Y_t6110.F10 = 'S' ");
            ...
            return selectPaging(connection, new ArrayParser<PropertyStatisticsEntity>()
            {
                    ...
                    throws SQLException
                {
                    ...
                    while (resultSet.next())
                    {
                        ...
                        record.riskAssureAmount = resultSet.getBigDecimal(17);
                        record.advanceAmount = resultSet.getBigDecimal(18);
                        record.advanceUnpaidAmount = resultSet.getBigDecimal(19);
                        BigDecimal zqzryk = resultSet.getBigDecimal(15);
                        BigDecimal tzglf = resultSet.getBigDecimal(16);
                        BigDecimal wyj = resultSet.getBigDecimal(20);
                        BigDecimal lx =
    ...
    {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT IFNULL(SUM(temp.balance),0),IFNULL(SUM(temp.freezeFunds),0),IFNULL(SUM(temp.tzzc),0),IFNULL(SUM(temp.loanAmount),0),IFNULL(SUM(temp.advanceAmount),0),IFNULL(SUM(temp.advanceUnpaidAmount),0) FROM");
        ...
        sql.append(" AND Y_t6110.F13 = 'F' AND Y_t6110.F10 = 'S') AS temp WHERE 1=1");
        BigDecimal balanceMin = query.balanceMin;
        if (balanceMin != null)
        {
            sql.append(" AND temp.balance >= ?");
            ...
            return select(connection, new ItemParser<PropertyStatisticsEntity>()
            {
                    ...
                    throws SQLException
                {
                    ...
                    if (resultSet.next())
                    {
                        ...
                        count.advanceAmount = resultSet.getBigDecimal(5);
                        count.advanceUnpaidAmount = resultSet.getBigDecimal(6);
                    }
                    return count;
                }
        ...
        throws Throwable
    {
        ...
        try (BufferedWriter out = new BufferedWriter(new OutputStreamWriter(outputStream, charset)))
        {
            ...
            for (PropertyStatisticsEntity qyMoneyEntity : qyMoneyEntitys)
            {
                ...
                writer.write(qyMoneyEntity.balance.add(qyMoneyEntity.freezeFunds));
