...
<%@page import="com.dimeng.p2p.console.servlets.finance.fuyou.dzgl.badclaimdzgl.BadclaimQuery"%>
...
<div class="right-container">
    <div class="viewFramework-body">
      <div class="viewFramework-content">
                <form action="<%=controller.getURI(request, BadclaimList.class)%>" method="post" name="form1" id="form1">
                    <div class="p20">
                        <div class="border">
                            <div class="title-container">
                                <i class="icon-i w30 h30 va-middle title-left-icon"></i>不良债权购买对账
                            </div>
                        ...
                        <div class="border mt20 table-container">
                            <table class="table table-style gray6 tl">
                                ...
                                <tbody class="f12">
                                <%
                                    TbdzEntity[] zqzrdzs = result.getItems();
                                    if (zqzrdzs.length > 0) {
                                        int i = 1;
                                        for (TbdzEntity zqzrdz : zqzrdzs) {
                                            if (zqzrdz == null) {
                                                continue;
                                            }
                                %>
                                <tr>
                                    ...
                                    <td class="tc"><%=zqzrdz.F01 %>
                                    </td>
                                    <td class="tc"><%=zqzrdz.userName %>
                                    </td>
                                    <td class="tc"><%=Formater.formatAmount(zqzrdz.amount) %>
                                    </td>
                                    <td class="tc"><%=DateTimeParser.format(zqzrdz.F04, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%if (zqzrdz.F06 != null) { %>
                                    <td class="tc"><%=DateTimeParser.format(zqzrdz.F06, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%} else { %>
                                    <td class="tc">&nbsp;</td>
                                    <%} %>
                                    <td class="tc"><%=zqzrdz.F10%>
                                    </td>
                                    <td class="tc"><%=zqzrdz.F03.getChineseName()%>
                                    </td>
                                    <td class="tc">
                                    	<% if ("S".equals(zqzrdz.F11.name())) { %>
                                    		已对账
                                    		<%} else { if (dimengSession.isAccessableResource(BadclaimQuery.class)) { %>
												<a target="_blank" onclick="zdOrderNo(<%=zqzrdz.F01%>)" class="blue ml5">去对账 </a> 
				                            <%}else{ %>
				                            	<span class="disabled">去对账</span>
			                            <%} } %> 
                                    </td>
                                   	<td class="tc"  title="<%=zqzrdz.F12%>"> 
...
<script type="text/javascript">
		function zdOrderNo(orderNo){
			$.ajax({
				type:"post",
				url:"/console/finance/fuyou/dzgl/badclaimdzgl/badclaimQuery.htm?orderNo=" + escape(orderNo),
