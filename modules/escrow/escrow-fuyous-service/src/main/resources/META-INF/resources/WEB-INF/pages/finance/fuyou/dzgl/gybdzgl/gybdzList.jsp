...
<%@page import="com.dimeng.p2p.console.servlets.finance.fuyou.dzgl.gybdzgl.GybQuery"%>
...
<%
    ...
    CURRENT_SUB_CATEGORY = "GYBDZ";
    PagingResult<TbdzEntity> result = (PagingResult<TbdzEntity>) request.getAttribute("result");
%>
<div class="right-container">
    <div class="viewFramework-body">
      <div class="viewFramework-content">
                <form action="<%=controller.getURI(request, GybdzList.class)%>" method="post" name="form1" id="form1">
                    <div class="p20">
                        <div class="border">
                            <div class="title-container">
                                <i class="icon-i w30 h30 va-middle title-left-icon"></i>公益捐款对账
                            </div>
                            <div class="content-container pl40 pt30 pr40">
                                <ul class="gray6 input-list-container clearfix">
                                    ...
                                    <li>
                                        <span class="display-ib mr5">投资时间：</span>
                        ...
                        <div class="border mt20 table-container">
                            <table class="table table-style gray6 tl">
                                ...
                                <tbody class="f12">
                                <%
                                    TbdzEntity[] gybdzs = result.getItems();
                                    if (gybdzs.length > 0) {
                                        int i = 1;
                                        for (TbdzEntity gybdz : gybdzs) {
                                            if (gybdz == null) {
                                                continue;
                                            }
                                %>
                                <tr>
                                    ...
                                    <td class="tc"><%=gybdz.F01 %>
                                    </td>
                                    <td class="tc"><%=gybdz.userName %>
                                    </td>
                                    <td class="tc"><%=Formater.formatAmount(gybdz.amount) %>
                                    </td>
                                    <td class="tc"><%=DateTimeParser.format(gybdz.F04, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%if (gybdz.F06 != null) { %>
                                    <td class="tc"><%=DateTimeParser.format(gybdz.F06, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%} else { %>
                                    <td class="tc">&nbsp;</td>
                                    <%} %>
                                    <td class="tc"><%=gybdz.F10%>
                                    </td>
                                    <td class="tc"><%=gybdz.F03.getChineseName()%>
                                    </td>
                                    <td class="tc">
                                    	<% if ("S".equals(gybdz.F11.name())) { %>
                                   			已对账
                                   		<%} else { if (dimengSession.isAccessableResource(GybQuery.class)) { %>
											<a target="_blank" onclick="zdOrderNo(<%=gybdz.F01%>)" class="blue ml5">去对账 </a> 
			                            <%}else{ %>
			                            	<span class="disabled">去对账</span>
			                            <%} } %> 
                                    </td>
                                    <td class="tc"  title="<%=gybdz.F12%>"> 
...
<script type="text/javascript">
    function zdOrderNo(orderNo){
		$.ajax({
			type:"post",
			url:"/console/finance/fuyou/dzgl/gybdzgl/gybQuery.htm?orderNo=" + escape(orderNo),
