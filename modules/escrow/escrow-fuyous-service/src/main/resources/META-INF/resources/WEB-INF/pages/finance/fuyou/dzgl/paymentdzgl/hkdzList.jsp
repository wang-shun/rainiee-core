...
<%@page import="com.dimeng.p2p.console.servlets.finance.fuyou.dzgl.paymentdzgl.PaymentQuery"%>
...
<div class="right-container">
    <div class="viewFramework-body">
      <div class="viewFramework-content">
                <form action="<%=controller.getURI(request, HkdzList.class)%>" method="post" name="form1" id="form1">
                    <div class="p20">
                        <div class="border">
                            <div class="title-container">
                                <i class="icon-i w30 h30 va-middle title-left-icon"></i>还款对账
                            </div>
                        ...
                        <div class="border mt20 table-container">
                            <table class="table table-style gray6 tl">
                                ...
                                <tbody class="f12">
                                <%
                                    TbdzEntity[] hkdzs = result.getItems();
                                    if (hkdzs.length > 0) {
                                        int i = 1;
                                        for (TbdzEntity hkdz : hkdzs) {
                                            if (hkdz == null) {
                                                continue;
                                            }
                                %>
                                <tr>
                                    ...
                                    <td class="tc"><%=hkdz.F01 %>
                                    </td>
                                    <td class="tc"><%=hkdz.userName %>
                                    </td>
                                    <td class="tc"><%=Formater.formatAmount(hkdz.amount) %>
                                    </td>
                                    <td class="tc"><%=DateTimeParser.format(hkdz.F04, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%if (hkdz.F06 != null) { %>
                                    <td class="tc"><%=DateTimeParser.format(hkdz.F06, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%} else { %>
                                    <td class="tc">&nbsp;</td>
                                    <%} %>
                                    <td class="tc"><%=hkdz.F10%>
                                    </td>
                                    <td class="tc"><%=hkdz.F03.getChineseName()%>
                                    </td>
                                    <td class="tc">
                                    	<% if ("S".equals(hkdz.F11.name())) { %>
                                   			已对账
                                   		<%} else { if (dimengSession.isAccessableResource(PaymentQuery.class)) { %>
											<a target="_blank" onclick="zdOrderNo(<%=hkdz.F01%>)" class="blue ml5">去对账 </a> 
			                            <%}else{ %>
			                            	<span class="disabled">去对账</span>
			                            <%} } %> 
                                    </td>
                                    <td class="tc"  title="<%=hkdz.F12%>"> 
...
<script type="text/javascript">
    function zdOrderNo(orderNo){
		$.ajax({
			type:"post",
			url:"/console/finance/fuyou/dzgl/paymentdzgl/paymentQuery.htm?orderNo=" + escape(orderNo),
