...
<%@page import="com.dimeng.p2p.console.servlets.finance.fuyou.dzgl.biddzgl.BidQuery"%>
...
<body>
    ...
    <div class="viewFramework-body">
      <div class="viewFramework-content">
					<form action="<%=controller.getURI(request, TbdzList.class)%>"
						method="post" name="form1" id="form1">
						<div class="p20">
							<div class="border">
								<div class="title-container">
									<i class="icon-i w30 h30 va-middle title-left-icon"></i>投资对账
								</div>
									...
									<ul class="gray6 input-list-container clearfix">
										...
										<li><span class="display-ib mr5">状态：</span> <select
											name="state" class="border mr20 h32 mw100">
												...
												<option value="DTJ"
													<%if ("DTJ".equals(request.getAttribute("state"))) {%>
													selected="selected" <%}%>>待提交</option>
												<option value="YTJ"
													<%if ("YTJ".equals(request.getAttribute("state"))) {%>
													selected="selected" <%}%>>已提交</option>
												<option value="DQR"
													<%if ("DQR".equals(request.getAttribute("state"))) {%>
													selected="selected" <%}%>>待确认</option>
												<option value="CG"
													<%if ("CG".equals(request.getAttribute("state"))) {%>
													selected="selected" <%}%>>成功</option>
												<option value="SB"
							...
							<div class="border mt20 table-container">
								<table class="table table-style gray6 tl">
									...
									<tbody class="f12">
										<%
										    TbdzEntity[] tbdzs = result.getItems();
										    	if (tbdzs.length > 0) {
										      	 	int i = 1;
										    	for (TbdzEntity tbdz : tbdzs) {
										         	if (tbdz == null) {
										           	continue;
										     	}
										%>
										<tr>
		                                	<td class="tc"><%=i++%></td>
											<td class="tc"><%=tbdz.F01%></td>
											<td class="tc"><%=tbdz.userName%></td>
											<td class="tc"><%=Formater.formatAmount(tbdz.amount)%></td>
											<td class="tc"><%=DateTimeParser.format(tbdz.F04, "yyyy-MM-dd HH:mm")%>
											</td>
											<%
											    if (tbdz.F06 != null) {
											%>
											<td class="tc"><%=DateTimeParser.format(tbdz.F06, "yyyy-MM-dd HH:mm")%>
											</td>
											<%
											    } else {
											%>
											...
											%>
											<td class="tc"><%=tbdz.F10%></td>
											<td class="tc"><%=tbdz.F03.getChineseName()%></td>
											<td class="tc">
	                                    	<% if ("S".equals(tbdz.F11.name())) { %>
                                    			已对账
                                    		<%} else { if (dimengSession.isAccessableResource(BidQuery.class)) { %>
												<a target="_blank" onclick="zdOrderNo(<%=tbdz.F01%>)" class="blue ml5">去对账 </a> 
				                            <%}else{ %>
				                            	<span class="disabled">去对账</span>
				                            <%} } %> 
	                                    	</td>
	                                    	<td class="tc"  title="<%=tbdz.F12%>"> 
...
<script type="text/javascript">
	function zdOrderNo(orderNo){
		$.ajax({
			type:"post",
			url:"/console/finance/fuyou/dzgl/biddzgl/bidQuery.htm?orderNo=" + escape(orderNo),
