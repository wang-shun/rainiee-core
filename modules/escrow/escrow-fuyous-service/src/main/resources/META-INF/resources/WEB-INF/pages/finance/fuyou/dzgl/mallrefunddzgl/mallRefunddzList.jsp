...
<%@page import="com.dimeng.p2p.console.servlets.finance.fuyou.dzgl.mallrefunddzgl.MallRefundQuery"%>
...
<div class="right-container">
    <div class="viewFramework-body">
      <div class="viewFramework-content">
                <form action="<%=controller.getURI(request, MallRefunddzList.class)%>" method="post" name="form1" id="form1">
                    <div class="p20">
                        <div class="border">
                            <div class="title-container">
                                <i class="icon-i w30 h30 va-middle title-left-icon"></i>商品退款对账
                            </div>
                        ...
                        <div class="border mt20 table-container">
                            <table class="table table-style gray6 tl">
                                ...
                                <tbody class="f12">
                                <%
                                    TbdzEntity[] mallrefunds = result.getItems();
                                    if (mallrefunds.length > 0) {
                                        int i = 1;
                                        for (TbdzEntity mallrefund : mallrefunds) {
                                            if (mallrefund == null) {
                                                continue;
                                            }
                                %>
                                <tr>
                                    ...
                                    <td class="tc"><%=mallrefund.F01 %>
                                    </td>
                                    <td class="tc"><%=mallrefund.userName %>
                                    </td>
                                    <td class="tc"><%=Formater.formatAmount(mallrefund.amount) %>
                                    </td>
                                    <td class="tc"><%=DateTimeParser.format(mallrefund.F04, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%if (mallrefund.F06 != null) { %>
                                    <td class="tc"><%=DateTimeParser.format(mallrefund.F06, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%} else { %>
                                    <td class="tc">&nbsp;</td>
                                    <%} %>
                                    <td class="tc"><%=mallrefund.F10%>
                                    </td>
                                    <td class="tc"><%=mallrefund.F03.getChineseName()%>
                                    </td>
                                    <td class="tc">
                                    	<% if ("S".equals(mallrefund.F11.name())) { %>
                                   			已对账
                                   		<%} else { if (dimengSession.isAccessableResource(MallRefundQuery.class)) { %>
											<a target="_blank" onclick="zdOrderNo(<%=mallrefund.F01%>)" class="blue ml5">去对账 </a> 
			                            <%}else{ %>
			                            	<span class="disabled">去对账</span>
			                            <%} } %> 
                                    </td>
                                    <td class="tc"  title="<%=mallrefund.F12%>"> 
...
<script type="text/javascript">
    function zdOrderNo(orderNo){
		$.ajax({
			type:"post",
			url:"/console/finance/fuyou/dzgl/mallrefunddzgl/mallRefundQuery.htm?orderNo=" + escape(orderNo),
