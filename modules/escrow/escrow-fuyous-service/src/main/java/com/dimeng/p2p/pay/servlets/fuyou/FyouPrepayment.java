...
public class FyouPrepayment extends AbstractFuyouServlet
{
    ...
    protected void processPost(HttpServletRequest request, HttpServletResponse response, ServiceSession serviceSession)
        throws Throwable
    {
        logger.info("富友托管-提前还款-开始——IP:" + request.getRemoteAddr());
        ...
        StringBuffer orderIdsStr = new StringBuffer();
        for (int i = 0; i < orderIds.length; i++)
        {
            ...
            {
                orderIdsStr.append(orderIds[i]).append(",");
            }
        }
        params.put("orderIdsStr", orderIdsStr.toString());
        try
        {
        ...
        String url = getConfigureProvider().format(URLVariable.PAY_PERPAYMENT_URL_SECOND);
        if ("fail".equals(params.get("hint")))
        {
            getController().prompt(request, response, PromptLevel.ERROR, "提前还款失败！");
            manage.writeFrontLog(FrontLogType.TQHK.getName(), "提前还款失败-标Id:" + id);
        }
        else
        {
            getController().prompt(request, response, PromptLevel.INFO, "恭喜您，提前还款成功！");
