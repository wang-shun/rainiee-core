...
{
    private static final Logger logger = Logger.getLogger(AdvanceTask.class);
    
    public void taskAdvance()
    {
        logger.info("开始执行【垫付定时自动对账】任务，开始时间：" + new java.util.Date());
        ...
        try (ServiceSession serviceSession = serviceProvider.createServiceSession())
        {
            ...
            List<T6501> list = autoManage.searchFailedOrder(OrderType.ADVANCE.orderType(), timestamp, number);
            if (list == null)
            {
                return;
            }
            FYPTAdvanceExecutor PTExecutor = resourceProvider.getResource(FYPTAdvanceExecutor.class);
            FYAdvanceExecutor JGExecutor = resourceProvider.getResource(FYAdvanceExecutor.class);
            ...
            for (T6501 t6501 : list)
            {
                 ...
                 */
                autoManage.autoAdvanceDZ(PTExecutor, JGExecutor, serviceSession, t6501, params);
        ...
        {
            logger.info("结束执行【垫付定时自动对账】任务，结束时间：" + new java.util.Date());
        }
    }
}
