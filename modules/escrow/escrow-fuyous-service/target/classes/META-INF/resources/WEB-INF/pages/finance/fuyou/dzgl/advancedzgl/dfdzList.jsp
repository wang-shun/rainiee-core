...
<%@page import="com.dimeng.p2p.console.servlets.finance.fuyou.dzgl.advancedzgl.AdvanceQuery"%>
...
<div class="right-container">
    <div class="viewFramework-body">
      <div class="viewFramework-content">
                <form action="<%=controller.getURI(request, DfdzList.class)%>" method="post" name="form1" id="form1">
                    <div class="p20">
                        <div class="border">
                            <div class="title-container">
                                <i class="icon-i w30 h30 va-middle title-left-icon"></i>垫付对账
                            </div>
                        ...
                        <div class="border mt20 table-container">
                            <table class="table table-style gray6 tl">
                                ...
                                <tbody class="f12">
                                <%
                                    TbdzEntity[] dfdzs = result.getItems();
                                    if (dfdzs.length > 0) {
                                        int i = 1;
                                        for (TbdzEntity dfdz : dfdzs) {
                                            if (dfdz == null) {
                                                continue;
                                            }
                                %>
                                <tr>
                                    ...
                                    <td class="tc"><%=dfdz.F01 %>
                                    </td>
                                    <td class="tc"><%=dfdz.userName %>
                                    </td>
                                    <td class="tc"><%=Formater.formatAmount(dfdz.amount) %>
                                    </td>
                                    <td class="tc"><%=DateTimeParser.format(dfdz.F04, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%if (dfdz.F06 != null) { %>
                                    <td class="tc"><%=DateTimeParser.format(dfdz.F06, "yyyy-MM-dd HH:mm")%>
                                    </td>
                                    <%} else { %>
                                    <td class="tc">&nbsp;</td>
                                    <%} %>
                                    <td class="tc"><%=dfdz.F10%>
                                    </td>
                                    <td class="tc"><%=dfdz.F03.getChineseName()%>
                                    </td>
                                    <td class="tc">
                                    	<% if ("S".equals(dfdz.F11.name())) { %>
                                    		已对账
                                    		<%} else { if (dimengSession.isAccessableResource(AdvanceQuery.class)) { %>
												<a target="_blank" onclick="zdOrderNo(<%=dfdz.F01%>)" class="blue ml5">去对账 </a> 
				                            <%}else{ %>
				                            	<span class="disabled">去对账</span>
			                            <%} } %> 
                                    </td>
                                    <td class="tc"  title="<%=dfdz.F12%>"> 
...
<script type="text/javascript">
    function zdOrderNo(orderNo){
		$.ajax({
			type:"post",
			url:"/console/finance/fuyou/dzgl/advancedzgl/advanceQuery.htm?orderNo=" + escape(orderNo),
