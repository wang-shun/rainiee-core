...
import com.dimeng.p2p.modules.financing.user.service.AgreementManage;
...
public class AgreementManageImpl extends AbstractFinancingManage implements
		AgreementManage {
	...
	public Organization getOrganization(int creditId) throws Throwable {
		...
		try (Connection connection = getConnection()) {
			...
			int orgId = 0;
			try (PreparedStatement ps = connection
					.prepareStatement("SELECT F02 FROM S70.T7031 WHERE F01=?")) {
			...
			try (PreparedStatement ps = connection
					.prepareStatement("SELECT F02,F04 FROM S70.T7029 WHERE F01=?")) {
				ps.setInt(1, orgId);
				try (ResultSet resultSet = ps.executeQuery()) {
					if (resultSet.next()) {
