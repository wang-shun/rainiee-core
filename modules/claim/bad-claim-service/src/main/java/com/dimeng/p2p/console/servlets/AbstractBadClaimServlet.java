/*
 * 文 件 名:  AbstractClaimServlet.java
 * 版    权:  深圳市迪蒙网络科技有限公司
 * 描    述:  <描述>
 * 修 改 人:  God
 * 修改时间:  2016年6月14日
 ...
 */
public class AbstractBadClaimServlet extends AbstractServlet
{
    
    protected static final Logger logger = Logger.getLogger(AbstractBadClaimServlet.class);
        ...
        throws IOException
    {
        ...
        if (result.getPageCount() > 1)
        {
            out.println("第<input type='text' id='" + idText
                + "' class='border ml5 mr5 w30 pl5 pr5' name='enterPage' onkeypress='enterPress(event)'  value=''/>页");
        ...
        throws ServletException, IOException
    {
        ...
        if (throwable instanceof AuthenticationException)
        {
            ...
            controller.redirectLogin(request, response, getResourceProvider().getResource(ConfigureProvider.class)
                .format(URLVariable.CONSOLE_LOGIN));
        }
        else
        {
            forwardView(request, response, getClass());
    ...
    protected void demo(final Envionment envionment)
        throws IOException
    {
        ...
        try
        {
            template.process(valueMap, new OutputStreamWriter(System.out, "utf-8"));
        }
