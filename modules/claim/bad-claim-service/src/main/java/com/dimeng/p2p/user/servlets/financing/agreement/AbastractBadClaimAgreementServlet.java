...
public class AbastractBadClaimAgreementServlet extends AbstractUserBadClaimServlet
{
    ...
    protected synchronized static void initFont(ServletContext servletContext)
    {
        ...
        Set<String> pathes = servletContext.getResourcePaths("/WEB-INF/font/");
        if (pathes != null)
        {
            for (String path : pathes)
            {
                FontFactory.register(servletContext.getRealPath(path));
            }
        }
    }
        ...
        throws Throwable
    {
        ...
        response.addHeader("Content-Disposition",
            "attachment; filename=" + String.format("%s.pdf", Long.toString(System.currentTimeMillis())));
        XMLWorker worker = null;
        final ServletContext servletContext = getServletContext();
        try (final ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
            final PrintWriter htmlWriter = new PrintWriter(new OutputStreamWriter(outputStream, charsetName));)
        {
            response.setCharacterEncoding(charsetName);
            RequestDispatcher dispatcher =
                request.getRequestDispatcher(getResourceProvider().getSystemDefine()
                    .getRewriter()
            ...
            htmlContext.setImageProvider(new AbstractImageProvider()
            {
                ...
                public Image retrieve(String src)
                {
                    ...
                    if (src.startsWith(contextPath))
                    {
                        ...
                        {
                            Image image =
                                Image.getInstance(new File(getServletContext().getRealPath("/"), src).toURI()
                                    .toString());
                            if (image != null)
                            {
                                store(src, image);
            ...
            cssResolver.setFileRetrieve(new FileRetrieve()
            {
                    ...
                    throws IOException
                {
                    ...
                    {
                        href = href.substring(getResourceProvider().getContextPath().length());
                        try (InputStreamReader reader =
                            new InputStreamReader(servletContext.getResourceAsStream(href), charsetName))
                        {
                            int i = -1;
                            while (-1 != (i = reader.read()))
                            {
