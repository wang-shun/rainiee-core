...
<%@page import="com.dimeng.p2p.console.servlets.finance.zjgl.yhdbgl.DbList" %>
<%@page import="com.dimeng.p2p.console.servlets.finance.zjgl.yhdbgl.DbRecordList" %>
<%@page import="com.dimeng.p2p.console.servlets.finance.zjgl.yhdbgl.ExportDbRecord" %>
<%@page import="com.dimeng.p2p.repeater.guarantor.entity.DbRecordEntity" %>
...
<%
    ...
    PagingResult<DbRecordEntity> result = (PagingResult<DbRecordEntity>) request.getAttribute("result");
...
<div class="right-container">
    <div class="viewFramework-body">
      <div class="viewFramework-content">
                <form id="searchForm" name="form1" action="<%=controller.getURI(request, DbRecordList.class)%>"
                      method="post">
                    <input type="hidden" name="id" value="<%=id %>">

                    <div class="p20">
                        <div class="border">
                            <div class="title-container"><i class="icon-i w30 h30 va-middle title-left-icon"></i>额度交易记录
                            </div>
                            <div class="content-container pl40 pt30 pr40">
                                <ul class="gray6 input-list-container clearfix">
                                    ...
                                    <li>
                                        <%if (dimengSession.isAccessableResource(ExportDbRecord.class)) {%>
                                    ...
                                    <li><a href="<%=controller.getURI(request, DbList.class)%>"
                        ...
                        <div class="border mt20 table-container">
                            <table class="table table-style gray6 tl">
                                ...
                                <tbody class="f12">
                                <%
                                    DbRecordEntity[] records = result.getItems();
                                    if (records != null && records.length > 0) {
                                        int i = 1;
                                        for (DbRecordEntity record : records) {
                                            if (record == null) {
                                                continue;
                                            }
                                %>
                                <tr class="tc">
                                    ...
                                    <td><%=DateTimeParser.format(record.F04, "yyyy-MM-dd HH:mm") %>
                                    </td>
                                    <td>
                                        <%=record.type %>
                                    </td>
                                    ...
                                    <td><%StringHelper.filterHTML(out, record.F08); %></td>
                                </tr>
                                <%
                                    }
                                } else {
...
<script type="text/javascript">
    function showExport() {
        document.getElementById("searchForm").action = "<%=controller.getURI(request, ExportDbRecord.class)%>";
        $("#searchForm").submit();
        document.getElementById("searchForm").action = "<%=controller.getURI(request, DbRecordList.class)%>";
    }
    $(function () {
