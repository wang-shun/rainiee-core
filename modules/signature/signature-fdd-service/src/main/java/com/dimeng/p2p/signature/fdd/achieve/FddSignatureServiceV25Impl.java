/*
 * 文 件 名:  FddSignatureServiceV25Impl.java
 * 版    权:  深圳市迪蒙网络科技有限公司
 * 描    述:  <描述>
 * 修 改 人:  guomianyun
 * 修改时间:  2017年10月26日
 ...
 */
public class FddSignatureServiceV25Impl extends AbstractFddService implements FddSignatureServiceV25
{
    private static final Logger logger = Logger.getLogger(FddSignatureServiceV25Impl.class);
    
    public FddSignatureServiceV25Impl(ServiceResource serviceResource)
    {
        ...
        try
        {
            try (Connection connection = getConnection())
            {
                ...
                if (null != t6110)
                {
                    ...
                    //客户信息修改接口请求
                    String result = FddClientV25.invokeInfoChange("", phoneNo, customerId);
    ...
    public String syncPersonAuto(int accountId)
    {
        try
        {
            ...
            try (Connection connection = getConnection())
            {
                ...
                String returnStr = FddClientV25.invokeSyncPersonAuto(realName, "", identifyId, mobile);
                ...
                if ("success".equals(result) && FDDReturnStatus.SUCCESS.code().equals(code))
                {
                    ...
                    String content = configureProvider.format(MsgVariavle.FDD_CA_APPLY_MSG, envionment);
                    sendLetter(connection, accountId, "法大大CA申请成功", content);
                    sendMsg(connection, mobile, content);
    ...
    public String uploadFile(int userId, int bidId, int orderId, T6273_F10 userType)
    {
        ...
        try
        {
            try (Connection connection = getConnection())
            {
                ...
                //合同上传接口请求
                String result = FddClientV25.invokeUploadDocs(contract_id, doc_title, file, doc_url, doc_type);
                ...
                if ("success".equals(resultUploadDoc) && FDDReturnStatus.SUCCESS.code().equals(code))
                {
                    ...
                    // if (userType != T6273_F10.JKR) // 借款人也可以自动签
                    // {
    ...
    private void extsignAuto(Connection connection, int userId, int bidId, int orderId, T6273_F10 userType)
    {
        try
        {
            ...
            if (null != t6273)
            {
                ...
                String position_type = "0"; //定位类型 0-关键字（默认）1-坐标 ,可以常量配置
                String keyword_strategy = "0"; //关键字签章策略：0：所有关键字签章 （默认）；  1：第一个关键字签章 ；  2：最后一个关键字签章；
                // 签个人
                String retStr =
                    FddClientV25.invokeExtSignAuto(transaction_id,
                        customer_id,
                        batch_id,
                        position_type,
                        client_role,
                ...
                {
                    //签平台章
                    retStr = this.extsignAutoPT(connection, t6230, t6273, orderId);
                    outobj = JSONObject.fromObject(retStr);
                    result = outobj.getString("result");
                    code = outobj.getString("code");
                    if ("success".equals(result) && FDDReturnStatus.SUCCESS.code().equals(code))
                    {
                        //更新签章状态、交易时间、交易请求号
     ...
     * @return
     */
    private String extsignAutoPT(Connection connection, T6230 t6230, T6273 t6273, int orderId)
        throws Throwable
    {
        try
        {
            ...
            if (null != t6273)
            {
                ...
                String notify_url = ""; //可选 服务器异步通知，如果指定，当签章完成后，法大大将通过此 URL 通知接入方平台
                String position_type = "0"; //定位类型 0-关键字（默认）1-坐标 ,可以常量配置
                String keyword_strategy = "1"; //关键字签章策略：0：所有关键字签章 （默认）；  1：第一个关键字签章 ；  2：最后一个关键字签章；
                String retStr =
                    FddClientV25.invokeExtSignAuto(transaction_id,
                        customer_id,
                        batch_id,
        ...
        throws Throwable
    {
        try (Connection connection = getConnection())
        {
            ...
            T6230 t6230 = selectT6230(connection, bidId);
            extsignAutoPT(connection, t6230, t6273, orderId);
        }
    }
        ...
        try
        {
            try (Connection connection = getConnection())
            {
                ...
                if (null != t6273)
                {
                    ...
                    String reqUrl =
                        FddClientV25.invokeExtSign("post",
                            transaction_id,
                            contract_id,
                            return_url,
                            client_type,
    ...
    public String contractFiling(String contract_id)
    {
        ...
        try
        {
            ...
            String returnStr = FddClientV25.invokeContractFilling(contract_id);
            JSONObject outobj = JSONObject.fromObject(returnStr);
            result = outobj.getString("result");
            String code = outobj.getString("code");
    ...
    public void viewContract(String contract_id)
    {
        try
        {
            String returnStr = FddClientV25.invokeViewContract(contract_id);
            JSONObject outobj = JSONObject.fromObject(returnStr);
            String result = outobj.getString("result");
            String code = outobj.getString("code");
        ...
        throws SQLException
    {
        ...
        try (Connection connection = getConnection())
        {
            try (PreparedStatement pstmt = connection.prepareStatement(sql.toString()))
            {
                ...
                pstmt.setString(5, T6273_F10.DFR.name());
                pstmt.setString(6, T6273_F10.BDFR.name());
                pstmt.setString(7, T6273_F10.JKR.name());
        ...
        throws Throwable
    {
        ...
        try
        {
            ...
            try (Connection connection = getConnection())
            {
                ...
                String returnStr = FddClientV25.invokeQueryStatus(t6273.F04, t6110.F20);
                JSONObject outobj = JSONObject.fromObject(returnStr);
                result = outobj.getString("result");
                String code = outobj.getString("code");
    ...
    @Override
    public String uploadFileNoSign(int userId, int bidId, int orderId, T6273_F10 userType)
        throws Throwable
    {
        String resultUploadDoc = "";
        try
    ...
    @Override
    public String extSignApp(int userId, int bidId, T6273_F10 userType, HttpServletResponse response, String returnUrl)
    {
        String reqUrl = null;
        try
        {
        ...
        throws Throwable
    {
        try (Connection connection = getConnection())
        {
            ...
            try (PreparedStatement ps = connection.prepareStatement("DELETE FROM S62.T6273 WHERE  F03=? AND F07=?"))
            {
                ps.setInt(1, bidId);
                ps.setString(2, T6273_F07.DQ.name());
                ps.execute();
            }
        }
    }
