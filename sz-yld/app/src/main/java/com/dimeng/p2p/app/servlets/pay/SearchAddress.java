...
public class SearchAddress extends AbstractAppServlet
{
        ...
        throws Throwable
    {
        ...
        // 获取区域信息
        if (tg && !StringHelper.isEmpty(prefix) && Prefix.SHUANGQIAN.name().equals(prefix.toUpperCase(Locale.ENGLISH)))
        {
            t5019s = getSQRegion(type, serviceSession, request);
        }
        else
        {
            t5019s = getNomalRegion(type, serviceSession, request);
        }
     ...
     * @throws Throwable 异常信息
     */
    private T5019[] getSQRegion(final String type, final ServiceSession serviceSession, final HttpServletRequest request)
        throws Throwable
    {
        SQDistrictManage disManage = serviceSession.getService(SQDistrictManage.class);
        T5019[] t5019s = null;
