...
], function(myOrderTemplate,myOrderList,PTRScroll) {
    var myOrderView = DMJS.DMJSView.extend({
        ...
        render: function() {
            ...
            this.noDestroy=false;
           	this.$el.html(_.template(myOrderTemplate,{}));
            ...
            self.myOrderList(self.page); 
            return this;
        },
       gotoOrderDetail:function(orderId){
       		DMJS.router.navigate("user/personal/orderDetail/"+orderId,true);
       },
       myOrderList:function(page,callBack){
        	var self=this;
        	var __call=function(){
        	...
        	self.controller.userModel.myOrder({'data':{pageIndex:page.pageIndex,pageSize:page.pageSize},'cancelLightbox':true,
        	"noCache":true,
				...
				var $dom=self.$el.find("div[item='listItem']");
				 if(!response.data||response.data.length<page.pageSize){
				    ...
				    self.list = response.data;
                     if(page.pageIndex == 1){
                     	$dom.html(_.template(myOrderList,self));
                     }
                     else{
                     	$dom.append(_.template(myOrderList,self));
                     }
                     page.pageIndex++;
                     //self.loadScroller();
			},
