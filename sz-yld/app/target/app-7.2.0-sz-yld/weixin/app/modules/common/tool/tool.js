...
define([], function(){
        var tool={
            ...
            changeTime:function(now,flg){//毫秒变时间
            	    now=new Date(now);
           ...
           */
           ...
           changeAmount:function(amount){ 
                ...
                if(r.test(amount) && parseInt(amount) < amount){  
                	if(amount<10000)return (amount*1).toFixed(2);
           ...
           _Navi_login:function(poinToURL){
           	var list=window.listHash;
           		if(list&&list.length>=2){
	       					if(list[list.length-1]=="user/personal/login"||list[list.length-1]=="user/personal/binLogin"){
           					DMJS.router.navigate(list[list.length-2],true);
           					list.length=list.length-1;
	       					}
	       					else{
