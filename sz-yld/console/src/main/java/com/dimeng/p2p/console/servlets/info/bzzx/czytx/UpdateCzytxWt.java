...
public class UpdateCzytxWt extends AbstractInformationServlet {
	...
	protected void processPost(final HttpServletRequest request,
		...
		try {
			...
			manage.updateQuestion(T5011_F02.CZYTX,question,IntegerParser.parse(request.getParameter("id")));
			sendRedirect(request, response,getController().getURI(request, SearchCzytxWt.class));
	...
	protected void processGet(HttpServletRequest request,
		...
		QuestionRecord record = manage.getQuestion(IntegerParser.parse(request
				.getParameter("id")),T5011_F02.CZYTX);
		if (record == null) {
