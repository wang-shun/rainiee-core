...
@Right(id = "P2P_C_SYS_ADDROLE", name = "新增", moduleId = "P2P_C_SYS_HTZH_YHZGL", order = 1)
public class AddRole extends AbstractSystemServlet
{
        ...
        throws Throwable
    {
        ...
        if (roleId > 0)
        {
            sendRedirect(request, response, getController().getURI(request, RoleList.class));
        }
    }
        ...
        throws ServletException, IOException
    {
        if (throwable instanceof LogicalException || throwable instanceof ParameterException)
        {
            ...
            sendRedirect(request, response, getController().getURI(request, AddRole.class));
        }
        else
        {
