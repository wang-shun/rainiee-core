...
@Right(id = "P2P_C_BASE_ADDPRODUCT", name = "新增标产品", moduleId = "P2P_C_BASE_OPTSETTINGS_PRODUCT")
public class AddProduct extends AbstractBaseServlet
{
        ...
        throws Throwable
    {
        ...
        // 校验最低起投金额
        BigDecimal minAmount = new BigDecimal("0"); //BigDecimalParser.parse(configureProvider.getProperty(SystemVariable.MIN_BIDING_AMOUNT));
        BigDecimal bmoneyRate =
            BigDecimalParser.parse(configureProvider.getProperty(SystemVariable.SUCCESS_BMONEY_RATE));
        BigDecimal tzRate = BigDecimalParser.parse(configureProvider.getProperty(SystemVariable.SUCCESS_TZ_RATE));
        BigDecimal faxiRate = BigDecimalParser.parse(configureProvider.getProperty(SystemVariable.YU_QI_FAXI_RATE));
        
        request.setAttribute("minAmount", minAmount);
        request.setAttribute("bmoneyRate", bmoneyRate);
        request.setAttribute("tzRate", tzRate);
        request.setAttribute("faxiRate", faxiRate);
        ...
        throws Throwable
    {
        ...
        {
            getController().prompt(request, response, PromptLevel.WARRING, "标产品名称已经存在");
            processGet(request, response, serviceSession);
            return;
        }
        if (null == t6216.F18)
