...
@Right(id = "P2P_C_BASE_ADDBANK", name = "新增银行", moduleId = "P2P_C_BASE_OPTSETTINGS_BANK")
public class AddBank extends AbstractBaseServlet
{
        ...
        throws Throwable
    {
        ...
        if (name == null)
        {
            getController().prompt(request, response, PromptLevel.WARRING, "银行名称不能为空");
            processGet(request, response, serviceSession);
            return;
        }
        if (manage.isBankExists(name))
        {
            getController().prompt(request, response, PromptLevel.WARRING, "银行名称已经存在");
            processGet(request, response, serviceSession);
            return;
        }
