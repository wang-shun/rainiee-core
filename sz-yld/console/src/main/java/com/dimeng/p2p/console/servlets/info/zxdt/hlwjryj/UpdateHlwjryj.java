...
public class UpdateHlwjryj extends AbstractInformationServlet
{
        ...
        throws Throwable
    {
        try
        {
            ...
            sendRedirect(request, response, getController().getURI(request, SearchHlwjryj.class));
        }
    ...
    {
        ArticleManage manage = serviceSession.getService(ArticleManage.class);
        ArticleRecord record = manage.get(IntegerParser.parse(request.getParameter("id")));
        if (record == null)
        {
        ...
        String content = manage.getContent(IntegerParser.parse(request.getParameter("id")));
