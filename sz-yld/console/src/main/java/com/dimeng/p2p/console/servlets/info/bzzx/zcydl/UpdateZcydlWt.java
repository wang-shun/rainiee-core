...
public class UpdateZcydlWt extends AbstractInformationServlet {
	...
	protected void processPost(final HttpServletRequest request,
		...
		try {
			...
			manage.updateQuestion(T5011_F02.ZCYDL,question,IntegerParser.parse(request.getParameter("id")));
			sendRedirect(request, response,getController().getURI(request, SearchZcydlWt.class));
	...
	protected void processGet(HttpServletRequest request,
		...
		QuestionRecord record = manage.getQuestion(IntegerParser.parse(request
				.getParameter("id")),T5011_F02.ZCYDL);
		if (record == null) {
