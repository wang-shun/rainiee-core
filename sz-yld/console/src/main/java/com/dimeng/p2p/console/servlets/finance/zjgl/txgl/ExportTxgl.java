package com.dimeng.p2p.console.servlets.finance.zjgl.txgl;
...
@Right(id = "P2P_C_FINANCE_EXPORTTXGL", name = "导出", moduleId = "P2P_C_FINANCE_ZJGL_TXGL", order = 3)
public class ExportTxgl extends AbstractFinanceServlet
{
        ...
        throws Throwable
    {
        ...
        PagingResult<UserWithdrawals> result = extraction.search(new TxglRecordQuery()
        {
            ...
            {
                return TimestampParser.parse(request.getParameter("startExtractonTime"));
            }
            
            @Override
            public Timestamp getEndExtractionTime()
            {
                return TimestampParser.parse(request.getParameter("endExtractionTime"));
            }
            
            @Override
            public int getBankId()
            {
                return IntegerParser.parse(request.getParameter("bankId"));
            }
            
            @Override
            public T6130_F09 getStatus()
            {
                return T6130_F09.DFK;
            }
        ...
        extraction.export(result.getItems(), response.getOutputStream(), "");
    }
}
