...
<%@page import="com.dimeng.p2p.console.servlets.account.vipmanage.grxx.HfjlView" %>
                ...
                <div class="home_main">
                    <div class="box box1 mb15">
                        ...
                        <div class="con con1">
                            ...
                            <ul class="yhxq_li">
                                <li>注册时间：  <%=TimestampParser.format(basicInfo.registrationTime)%>
                                </li>
                            </ul>
                            <div class="clear"></div>
                        </div>
                    ...
                    <div class="newsbox">
                        <div class="til clearfix">
                            <div class="Menubox">
                                <ul>
                                    <li id="one1"
                                        onclick="window.location.href='<%=controller.getURI(request, JbxxView.class)%>?userId=<%=basicInfo.userId%>'">
                                        基本信息
                                    </li>
                                    <li id="one2"
                                        onclick="window.location.href='<%=controller.getURI(request, ByrzView.class)%>?userId=<%=basicInfo.userId%>'">
                                        必要认证（<%=byrztg%>/<%=basicInfo.needAttestation%>）
                                    </li>
                                    <li id="one3"
                                        onclick="window.location.href='<%=controller.getURI(request, KxrzView.class)%>?userId=<%=basicInfo.userId%>'">
                                        可选认证（<%=kxrztg%>/<%=basicInfo.notNeedAttestation%>）
                                    </li>
                                    <li id="one5"
                                        onclick="window.location.href='<%=controller.getURI(request, JkjlView.class)%>?userId=<%=basicInfo.userId%>'">
                                        借款记录
                                    </li>
                                    <li id="one6"
                                        onclick="window.location.href='<%=controller.getURI(request, TbjlView.class)%>?userId=<%=basicInfo.userId%>'">
                                        投资记录
                                    </li>
                            ...
                            <form action="<%=controller.getURI(request, HfjlView.class)%>" method="post">
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="yhgl_table">
                                    ...
                                    <tr class="bom">
                                        ...
                                        <td title="<%StringHelper.filterHTML(out, entity.F03);%>"><%
                                            StringHelper.truncation(out, entity.F03, 20);%></td>
                                        <td><%=DateTimeParser.format(entity.F04)%>
                                        </td>
                                        <td><%=DateTimeParser.format(entity.F05)%>
                                        </td>
                                    </tr>
                                    <% }
                                    }%>
