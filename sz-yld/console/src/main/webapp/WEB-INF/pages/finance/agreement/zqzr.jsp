...
<div class="protocol">
    ...
    <p class="clearfix pt10"><a href="<%=controller.getURI(request, Zqzr.class)%>?id=<%=zcid%>&s=1"
                                class="btn btn01 fr">点击下载</a></p>
    <%} %>
    <div class="tit">
        <h2><strong>债权转让及受让协议</strong></h2>

        <p class="tr">编号：ZR-<%=zcid %>
        </p>
    </div>
    <div class="con">
        <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;本债权转让及受让协议（下称“本协议”）由以下双方于签署：</p>

        <div class="tab_one mt10 mb10">
            <table width="80%">
                <tr>
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;本债权转让及受让协议（下称“本协议”）由以下双方于签署：就甲方通过深圳市迪蒙网络科技有限公司（以下“<%StringHelper.filterHTML(out, siteName); %>
        ”系指深圳市迪蒙网络科技有限公司和下述<%StringHelper.filterHTML(out, siteName); %>
        网站的统称）运营管理的<%StringHelper.filterHTML(out, configureProvider.getProperty(SystemVariable.SITE_DOMAIN)); %>
        网站（下称“<%StringHelper.filterHTML(out, siteName); %>网站”）向乙方转让债权事宜，双方经协商一致，达成如下协议：<br/>
        ...
        <table width="80%" class="tab_two">
            ...
            <tr>
                <td>借款本金数额</td>
                <td>¥ <%=Formater.formatAmount(creditDetail.amount) %>
                </td>
            </tr>
            <tr>
                <td>借款年化利率</td>
            ...
            <tr>
                <td>原借款期限</td>
                <td><%=creditDetail.limitMonth %>个月，<%=c1.get(Calendar.YEAR) %>年<%=c1.get(Calendar.MONTH) + 1 %>
                    月<%=c1.get(Calendar.DAY_OF_MONTH) %>日起，
                    至<%=c2.get(Calendar.YEAR) %>年<%=c2.get(Calendar.MONTH) + 1 %>月<%=c2.get(Calendar.DAY_OF_MONTH) %>日止
                </td>
            </tr>
            <tr>
                <td>月偿还本息数额</td>
                <td>¥ <%=Formater.formatAmount(creditDetail.monthAmount) %>
                </td>
            </tr>
            <tr>
                <td>还款日</td>
                <td>自<%=c1.get(Calendar.YEAR) %>年<%=c1.get(Calendar.MONTH) + 1 %>月<%=c1.get(Calendar.DAY_OF_MONTH) %>
                    日起，每月 <%=c1.get(Calendar.DAY_OF_MONTH) %>日（24:00前，节假日不顺延）
                </td>
            </tr>
        </table>
        ...
        <table width="80%" class="tab_two">
            <tr>
                ...
                <td width="69%"><%=Formater.formatAmount(info.zqjz) %>
                </td>
            </tr>
            <tr>
                <td>转让价款</td>
                <td><%=Formater.formatAmount(info.zrjk) %>
                </td>
            </tr>
            <tr>
                <td>转让管理费</td>
                <td><%=Formater.formatAmount(info.zrglf) %>
                </td>
            </tr>
            <%
            ...
            <tr>
                <td>转让日期</td>
                <td><%=c3.get(Calendar.YEAR) %>年<%=c3.get(Calendar.MONTH) + 1 %>月<%=c3.get(Calendar.DAY_OF_MONTH) %>日
                </td>
            </tr>
            <tr>
                <td>剩余还款分期月数</td>
                <td><%=info.syqs %>个月，<%=c3.get(Calendar.YEAR) %>年<%=c3.get(Calendar.MONTH) + 1 %>
                    月<%=c3.get(Calendar.DAY_OF_MONTH) %>日起，
                    至<%=c2.get(Calendar.YEAR) %>年<%=c2.get(Calendar.MONTH) + 1 %>月<%=c2.get(Calendar.DAY_OF_MONTH) %>日止
                </td>
            </tr>
        </table>
        <br/>
        1.2 债权转让流程<br/>
        1.2.1 双方同意并确认，双方通过自行或授权有关方根据<%StringHelper.filterHTML(out, siteName); %>
        网站有关规则和说明，在<%StringHelper.filterHTML(out, siteName); %>网站进行债权转让和受让购买操作等方式确认签署本协议。<br/>
        1.2.2 双方接受本协议且<%StringHelper.filterHTML(out, siteName); %>
        审核通过时，本协议立即成立,并待转让价款支付完成时生效。协议成立的同时甲方不可撤销地授权<%StringHelper.filterHTML(out, siteName); %>
        自行或委托第三方支付机构或合作的金融机构，将转让价款在扣除甲方应支付给<%StringHelper.filterHTML(out, siteName); %>
        的转让手续费之后划转、支付给乙方，上述转让价款划转完成即视为本协议生效且标的债权转让成功；同时甲方不可撤销地授权<%StringHelper.filterHTML(out, siteName); %>
        将其代为保管的甲方与标的债权借款人签署的电子文本形式的《借款协议》（下称“借款协议”）及借款人相关信息在<%StringHelper.filterHTML(out, siteName); %>网站有关系统板块向乙方进行展示。<br/>
        1.2.3 本协议生效且标的债权转让成功后，双方特此委托<%StringHelper.filterHTML(out, siteName); %>
        将标的债权的转让事项及有关信息通过站内信等形式通知与标的债权对应的借款人。<br/><br/>
        1.3 自标的债权转让成功之日起，乙方成为标的债权的债权人，承继借款协议项下出借人的权利并承担出借人的义务。
        <h3>2. 保证与承诺</h3>
        2.1 甲方保证其转让的债权系其合法、有效的债权，不存在转让的限制。甲方同意并承诺按有关协议及<%StringHelper.filterHTML(out, siteName); %>
        网站的相关规则和说明向<%StringHelper.filterHTML(out, siteName); %>0支付债权转让管理费。<br/><br/>
        ...
        5.2 本协议及其修改或补充均通过<%StringHelper.filterHTML(out, siteName); %>
        网站以电子文本形式制成，可以有一份或者多份并且每一份具有同等法律效力；同时双方委托<%StringHelper.filterHTML(out, siteName); %>
        代为保管并永久保存在<%StringHelper.filterHTML(out, siteName); %>为此设立的专用服务器上备查。双方均认可该形式的协议效力。<br/><br/>
