...
<%@ page import="com.dimeng.p2p.console.servlets.finance.czgl.xxczgl.*" %>
<html>
<head>
                            ...
                            <div class="content-container pl40 pt30 pr40">
                                <ul class="gray6 input-list-container clearfix">
                                    ...
                                    <li>
                                        ...
                                        %>
                                        <a href="javascript:void(0)" onclick="showImport()"
                                           class="btn btn-blue radius-6 mr5  pl1 pr15"><i
                                    ...
                                    <li>
                                        <%
                                            if (dimengSession.isAccessableResource(ImportRecord.class)) {
                                        %>
                                        <a href="javascript:showImport();" id="importUser"
                                           class="btn btn-blue radius-6 mr5  pl10 pr10">批量导入</a>
                                        <a href="<%=controller.getStaticPath(request)%>/template.xls" class="blue">下载模板</a>
                                        <%} else { %>
                                        <span class="btn btn-gray radius-6 mr5 pl10 pr10">批量导入</span>
                                        <%} %>
                                    </li>
                                </ul>
                            </div>
...
<div id="import" style="display: none">
    <div class="popup-box import-box" style="min-height: 150px;">
        ...
        <form id="import_form" action="<%=controller.getURI(request,ImportRecord.class)%>" name="form2" class="form2"
            ...
            <div class="popup-content-container pt20 ob20 clearfix">
                <div class="gray6">
                    <ul>
                        ...
                        <li class="mb10"><span class="display-ib tr mr5" style="width:85px;">&nbsp;</span>支持导入xls、xlsx格式数据，不得超过50条数据
                        </li>
                    </ul>
                </div>
                <div class="tc f16">
                    <a href="javascript:$('#import_form').submit();" name="save" formname="form2"
...
<form action="<%=controller.getURI(request,WrongRecordList.class)%>" id="errorForm" name="errorForm" target="_blank" method="post">
    <input type="hidden" name="jsonStr" value='<%=(String)request.getAttribute("errorRecords")%>'/>
</form>
<%
    String warringMessage = controller.getPrompt(request, response, PromptLevel.WARRING);
    if (!StringHelper.isEmpty(warringMessage)) {
        String errorRecords = (String)request.getAttribute("errorRecords");

%>
<script type="text/javascript">
    $("div.popup_bg").show();
    <%if(!StringHelper.isEmpty(errorRecords)){%>
    $("#info").html(showDialogInfo('导入失败，模板内容包含错误信息，请核对后再进行导入，<a href="javascript:void(0)" onclick="showRecord()" class="blue">查看错误信息</a>', "wrong"));
    <%}else{%>
...
<script type="text/javascript">
  ...
  //提现审核时确认放款后提示信息，中间跳转操作
    ...
    function showRecord(){
        $("#import_form").attr("action","<%=controller.getURI(request, XxczglList.class)%>");
        $("#errorForm").submit();
        $("#import_form").submit();
        //$("#errorForm").attr("","");
    }
