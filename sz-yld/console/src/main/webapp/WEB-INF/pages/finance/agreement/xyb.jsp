...
<div>
    ...
    <p class="clearfix pt10"><a target="_blank" href="<%=controller.getURI(request, Xyb.class)%>?id=<%=creditId%>&s=1"
                                class="btn ">点击下载</a></p>
    <%} %>
    <div class="tit">
    ...
    %><br/><br/>
        ...
        <h3>2. 协议的订立及支付</h3>
        2.1 各方同意并确认，各方通过自行或授权有关方根据<%=siteName%>有关规则和说明，在<%=siteName%>进行借款申请和投资操作等方式确认签署本协议。<br/><br/>
        ...
        3.2.3
        乙方确认甲方可以根据自己的意愿对其根据本协议形成的对乙方的部分或全部债权进行转让，并特此同意甲方届时转让债权无需另行通知乙方；同时乙方承诺若届时需要，将采取必要措施配合债权的一次或多次转让或届时债权人债权的实现。在债权转让后，甲方在本协议项下的权利和义务自动转移到债权受让人名下，乙方应按照本协议的约定向债权受让人支付每期应还借款本息，不得以未接到债权转让通知为由拒绝履行还款义务。<br/>
        ...
        3.3.4
        甲乙双方同意丙方有权代甲方在必要时对乙方进行贷款的违约提醒及催收工作，包括但不限于电话通知、上门通知、发律师函、对乙方提起诉讼等。甲方在此确认委托丙方为其进行以上工作，并授权丙方可以将此工作委托给其他方进行。乙方对前述委托的提醒、催收事项已明确知晓并应积极配合，同时应承担因乙方逾期还款而产生的催收费用。<br/>
        ...
        4.2
        对于丙方向乙方提供的一系列信用服务，乙方同意在本协议生效时向丙方支付本协议第1条所约定借款本金总额的<%=Formater.formatRate(new BigDecimal(configureProvider.getProperty(SystemVariable.LMONEY_SUCCESS_RATION)))%>
        (即人民币<%=Formater.formatAmount(new BigDecimal(configureProvider.getProperty(SystemVariable.LMONEY_SUCCESS_RATION)).multiply(creditDetail.amount))%>
        元)作为居间服务费，该“居间服务费”由甲乙双方授权并委托丙方根据本协议规定在划扣出借本金时一次性扣除支付给丙方，扣除后视为乙方已缴纳。在本协议约定的借款期限内，
        乙方应每月向丙方支付本协议第1条约定的借款本金总额的<%=Formater.formatRate(new BigDecimal(configureProvider.getProperty(SystemVariable.LMONEY_SUCCESS_RATION)))%>
        (即人民币<%=Formater.formatAmount(new BigDecimal(configureProvider.getProperty(SystemVariable.LMONEY_SUCCESS_RATION)).multiply(creditDetail.amount))%>
        元)，作为借款管理费用，共需支付期，共计人民币<%=Formater.formatAmount(new BigDecimal(configureProvider.getProperty(SystemVariable.LMONEY_SUCCESS_RATION)).multiply(creditDetail.amount).multiply(new BigDecimal(creditDetail.limitMonth)))%>
        ...
        5.2
        任何一方违约，违约方应承担因违约使得其他各方产生的费用和损失，包括但不限于调查、诉讼费、律师费等，应由违约方承担。如违约方为乙方的，甲方有权立即解除本协议，并要求乙方立即偿还未偿还的本金、利息、罚息。此时，乙方还应向丙方支付所有应付的借款管理费。<br/><br/>
        ...
        <br/>
        5.7
        如果乙方逾期支付任何一期还款超过30天，或连续逾期三期以上（含三期），或累计逾期达五期以上（含五期），或甲方/丙方发现乙方出现逃避、拒绝沟通或拒绝承认欠款事实、故意转让资金、信用情况恶化等危害本协议借款的情形，则本协议项下的全部借款本息提前到期，同时：<br/>
        ...
        8.3 本协议及其修改或补充均采用通过<%=siteName%>
        以电子文本形式制成，可以有一份或者多份并且每一份具有同等法律效力，并永久保存在丙方为此设立的专用服务器上备查和保管。双方均认可该形式的协议效力。<br/><br/>
        8.4 如果本协议中的任何一条或多条违反适用的法律法规，则该条将被视为无效，但该无效条款并不影响本协议其他条款的效力。<br/><br/>
        8.5
        本协议签订之日起至借款本息和有关费用全部清偿之日止，甲方或乙方的下列信息如发生变更的，其应当在相关信息发生变更三日内将更新后的信息提供给丙方：本人、本人的家庭联系人及紧急联系人、工作单位、居住地址、住所电话、手机号码、电子邮箱、银行账户的变更。若因任何一方不及时提供上述变更信息而带来的损失或额外费用应由该方自行承担。<br/><br/>
