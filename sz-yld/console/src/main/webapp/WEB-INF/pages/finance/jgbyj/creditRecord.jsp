                ...
                <div class="home_main">
                    <div class="box box1 mb15">
                        ...
                        <div class="con">
                            ...
                            <ul class="yhxq_li">
                                <li>机构名称：<span
                                        class="blue"><%StringHelper.filterHTML(out, jgfxbyj.organizationName); %></span>
                                </li>
                            </ul>
                            <ul class="yhxq_li">
                                <li>机构信用金额：<span
                                        class="blue"><%=Formater.formatAmount(jgfxbyj.organizationCreditAmount) %></span>元
                                </li>
                            </ul>
                            <ul class="yhxq_li">
                                <li>机构可用信用总额：<span
                                        class="blue"><%=Formater.formatAmount(jgfxbyj.organizationUsableAmount) %></span>元
                                </li>
                            </ul>
                            <ul class="yhxq_li">
                                <li>
                    ...
                    <div class="box box1 mb15">
                        <div class="con">
                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="hwi">
                                <tr>
                                    <td width="30%">申请借款（笔）：<%=jgxyjl.sqjkCount %>
                                    </td>
                                    <td width="40%">
                                        信用额度（元）：<%=Formater.formatAmount(jgfxbyj.organizationCreditAmount) %>
                                    </td>
                                    <td width="30%">逾期金额（元）：<%=Formater.formatAmount(jgxyjl.yqjeAmount) %>
                                    </td>
                                </tr>
                                <tr>
                                    <td>成功借款（笔）：<%=jgxyjl.cgjkCount %>
                                    </td>
                                    <td>借款总额（元）：<%=Formater.formatAmount(jgxyjl.jkTotalAmount) %>
                                    </td>
                                    <td>逾期次数（次）：<%=jgxyjl.yqcsCount %>
                                    </td>
                                </tr>
                                <tr>
                                    <td>还清笔数（笔）：<%=jgxyjl.hqbsCount %>
                                    </td>
                                    <td>待还本金（元）：<%=Formater.formatAmount(jgxyjl.dhbjAmount) %>
                                    </td>
                                    <td>严重逾期（笔）：<%=jgxyjl.yzyqCount %>
                                    </td>
                                </tr>
                            </table>
                            <div class="clear"></div>
        ...
        <div class="con">
            <form action="<%=controller.getURI(request, Tzxyd.class)%>" method="post" class="tzForm">
                ...
                <table class="jgzjcz_table">
                    <tr>
                        <td>可用信用额度:</td>
                        <td><%=Formater.formatAmount(jgfxbyj.organizationUsableAmount)%>
                        </td>
                    </tr>
                    <tr>
                        <td>调整可用信用额度:</td>
                        <td>
                            <input type="text" name="ammount" class="yhgl_input required max-length-10"
