...
<%
    Ptzjgl ptzjgl = ObjectHelper.convert(request.getAttribute("ptzjgl"), Ptzjgl.class);
    PagingResult<PtzjglRecord> result = (PagingResult<PtzjglRecord>) request.getAttribute("result");
...
<div class="w_main">
    <div class="main clearfix">
        <div class="wrap">
            <div class="r_main">
                <form action="<%=controller.getURI(request, PtzjglList.class) %>" name="form1" method="post"
                      id="searchForm">
                    <div class="home_main">
                            ...
                            <div class="con">
                                <div class="admin_input admin_pb">
                                    <div class="table">
                                        ...
                                        <p>
                                            <span>时间：</span>
                                            <input type="text" name="startPayTime" id="datepicker1" readonly="readonly"
                                                   class="yhgl_input date"
                                                   value="<%StringHelper.filterHTML(out, request.getParameter("startPayTime")); %>"/>至
                                            <input type="text" name="endPayTime" id="datepicker2" readonly="readonly"
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="3" class="yhgl_table tc">
                            ...
                            <tr class="dhsbg">
                                ...
                                <td><%=DateTimeParser.format(record.playTime, "yyyy-MM-dd HH:mm:ss") %>
                                </td>
                                <td><%if (!StringHelper.isEmpty(record.loginName)) {%><%
                                    StringHelper.filterHTML(out, record.loginName); %><%} else { %>--<%} %></td>
                                <td><%if (record.type != null) {%><%=record.type.getName() %><%} %></td>
                                <td><%=Formater.formatAmount(record.income) %>
                                </td>
                                <td><%=Formater.formatAmount(record.replay) %>
                                </td>
                                <td><%=Formater.formatAmount(record.remain) %>
                                </td>
                                <td title="<%StringHelper.filterHTML(out, record.remark);%>"><%
                                    StringHelper.filterHTML(out, StringHelper.truncation(record.remark, 15)); %></td>
                            </tr>
                            <%
                                }
                            } else {
            ...
            <form action="<%=controller.getURI(request, WithdrawalPtzj.class)%>" method="post" class="withdrawalForm">
                ...
                <table class="jgzjcz_table">
                    <tr>
                        <td>提现金额:</td>
                        <td>
                            <input type="text" id="tx" name="ammount" onblur="txje()" class="yhgl_input required"
                                   mtest="/^[-\+]?\d+(\.\d+)?$/" mtestmsg="金额格式不正确"/>
