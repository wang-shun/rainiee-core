...
<div class="w_main">
    <div class="main clearfix">
        <div class="wrap">
            <div class="r_main">
                <form action="<%controller.getURI(request, JgbyjList.class);%>" name="form1" method="post"
                      id="searchForm">
                    <div class="home_main">
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="3" class="yhgl_table tc">
                            ...
                            <tr class="dhsbg">
                                ...
                                <td>
                                    ...
                                    %>
                                    <a href="<%=controller.getURI(request, ByjDetail.class)%>?id=<%=item.id%>"
                                       class="blue">备用明细</a>
                                    <%} else { %>
                                    <span class="disabled">备用明细</span>
                                    <%} %>
                                    &nbsp;&nbsp;
                                    <%
                                        if (dimengSession.isAccessableResource(YwDetail.class)) {
                                    %>
                                    <a href="<%=controller.getURI(request, YwDetail.class)%>?id=<%=item.id%>"
                                       class="blue">业务明细</a>
                                    <%} else { %>
                                    <span class="disabled">业务明细</span>
                                    <%} %>
                                    &nbsp;&nbsp;
                                    <%
                                        if (dimengSession.isAccessableResource(CreditRecord.class)) {
                                    %>
                                    <a href="<%=controller.getURI(request, CreditRecord.class)%>?id=<%=item.id%>"
                                       class="blue">信用记录</a>
                                    <%} else { %>
                                    <span class="disabled">信用记录</span>
                                    <%} %>
                                    &nbsp;&nbsp;
                                    <%
                                        if (dimengSession.isAccessableResource(JgDfsdzqList.class)) {
                                    %>
                                    <a href="<%=controller.getURI(request, JgDfsdzqList.class)%>?id=<%=item.id %>"
                                       class="blue">垫付债权</a>
                                    <%} else { %>
                                    <span class="disabled">垫付债权</span>
                                    <%} %>
                                </td>
                            </tr>
                            <%
