...
<%@page import="com.dimeng.p2p.console.servlets.finance.agreement.Sdrz" %>
<%@page import="com.dimeng.p2p.modules.finance.console.service.entity.Organization" %>
...
<div class="protocol">
    ...
    <div class="con">
        ...
        <table width="80%" class="tab_two">
            ...
            <tr>
                ...
                %></td>
                <td>￥<%=Formater.formatAmount(joinedUser.amount)%>
                </td>
                <td><%=joinedUser.limitMonth%>
                </td>
                <td>￥ <%=Formater.formatAmount(joinedUser.monthAmount)%>
                </td>
            </tr>
            <%
                    }
            ...
            <tr>
                <td>总计</td>
                <td>￥<%=Formater.formatAmount(totalAmount)%>
                </td>
                <td>&nbsp;</td>
                <td>￥ <%=Formater.formatAmount(totalMonthAmount)%>
                </td>
            </tr>
        </table>
        <br/>
    ...
    %><br/><br/>
        ...
        <p align="left"><strong>鉴于： </strong><br/>
            1. 丁方是一家在深圳市合法成立并有效存续的有限责任公司，拥有<%=configureProvider.getProperty(SystemVariable.SITE_DOMAIN)%>
            网站（下称“<%=siteName%>”）的经营权，提供信用咨询，为交易提供信息服务 <br/>
            ...
            5. 丙方是一家在上海市合法成立并有效存续的有限责任公司；负责协助丁方，对乙方在<%=siteName%>平台上的借款申请进行调查核实（包括但不限于实地调查等）；协助乙方完成其在<%=siteName%>
            平台上借款信息的发布，承诺保证乙方信息的真实性并协助甲方进行本协议项下的借款回收及日常管理工作。 <br/>
            <strong>各方经协商一致，于</strong><strong>【<%=calendar.get(Calendar.YEAR)%>】年【<%=calendar.get(Calendar.MONTH) + 1%>
                】月【<%=calendar.get(Calendar.DAY_OF_MONTH)%>】日 </strong><strong>签订如下协议，共同遵照履行：</strong></p>
        <br/>
        ...
        <table width="80%" class="tab_two">
            ...
            <tr>
                <td><strong>借款年化利率</strong></td>
                <td><%=Formater.formatRate(creditDetail.rate)%>
                </td>
            </tr>
            <tr>
                <td><strong>借款期限</strong></td>
                <td><%=creditDetail.limitMonth%> 个月， <%=DateParser.format(creditDetail.startDate)%>
                    起，至 <%=DateParser.format(creditDetail.endDate)%> 止
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>自<%=DateParser.format(creditDetail.startDate)%> 起，每月<%=calendar.get(Calendar.DAY_OF_MONTH)%>日
        ...
        <p align="left">2.1 各方同意并确认，各方通过自行或授权有关方根据<%=siteName%>有关规则和说明，在<%=siteName%>进行借款申请和投资操作等方式确认签署本协议。 <br/>
        ...
        <p align="left"><strong>3.2 </strong><strong>乙方权利和义务</strong> <br/>
            ...
            3.2.3
            乙方确认甲方可以根据自己的意愿进行本协议下其对乙方的部分或全部债权的转让，并特此同意甲方届时转让债权无需另行通知乙方；同时承诺若届时需要，将采取必要措施配合债权的一次或多次转让或届时债权人债权的实现。在债权转让后，甲方在本协议项下的权利和义务转移到债权受让人名下，乙方应按照本协议的约定向债权受让人支付每期应还借款本息，不得以未接到债权转让通知为由拒绝履行还款义务。
            <br/>
        ...
        3.3.6
        丙方应在乙方逾期还款时，以其风险准备金为限先行垫付乙方应偿还给甲方的本息（下称“垫付的借款本息”），丙方在向甲方进行垫付的同时，即刻自动取得甲方对乙方的相应债权及其他权利，包括但不限于就前述债权向乙方追偿的权利，且甲方不再就前述已让渡给丙方的债权向乙方主张任何权利，或向丙方主张任何权利，同时甲方在此授权丙方或丁方可以视情况（必要时）将垫付等相关事项通知乙方。此后乙方应直接向丙方偿还其垫付的借款本息，并按照双方签署的《信用咨询及管理服务协议》向丙方支付垫付服务费用和逾期违约金。甲方同意并认可丙方前述垫付的行为并不代表有关逾期应归责于丙方，亦不代表丙方须为此承担其他任何责任。
        <br/>
        3.3.7 丙方按照本协议及其他有关协议的约定，将通过其合作的银行每月从乙方还款账户中划扣相应的数额；若出现非因乙方原因而多划扣的情形时，由丙方负责解决，丙方对多划扣的部分按照同期银行存款利息对乙方进行赔偿。
        <br/><br/>
        ...
        3.4.4
        甲乙双方同意丁方有权代甲方在必要时对乙方或丙方进行贷款的违约提醒及催收工作，包括但不限于电话通知、上门通知、发律师函、对乙方提起诉讼等。甲方在此确认委托丁方为其进行以上工作，并授权丁方可以将此工作委托给其他方进行。乙方和丙方对前述委托的提醒、催收事项已明确知晓并应积极配合，同时应承担因乙方逾期还款而产生的催收费用。
        <br/>
        ...
        （7）正常的本金； <br/>
        5.5
        乙方应严格履行还款义务，如乙方逾期还款且丙方未替乙方垫付应还本息时，则应按照下述条款向甲方支付逾期罚息，自逾期开始之后，逾期本金的正常利息停止计算；如乙方逾期还款但丙方替乙方先行垫付应还本息时，在甲方收到丙方代为垫付的应还本息后，甲方不再向乙方收取罚息。
        <p><br/>
            <br/>
        ...
        5.6
        乙方应严格履行还款义务，如乙方逾期还款且丙方未替乙方垫付应还本息时，则应按照下述条款向丁方支付逾期管理费；如乙方逾期还款但丙方替乙方先行垫付应还本息时，在甲方收到丙方代为垫付的应还本息后，丁方不再向乙方收取逾期管理费。
        <p><br/></p>
        ...
        <p align="left">5.7
            如果乙方逾期支付任何一期还款超过30天，或连续逾期三期以上（含三期），或累计逾期达五期以上（含五期），或甲方/丁方发现乙方出现逃避、拒绝沟通或拒绝承认欠款事实、故意转让资金、信用情况恶化等危害本协议借款的情形，则本协议项下的全部借款本息提前到期，同时：
            <br/>
            ...
            5.9
            本借款协议中的所有甲方与乙方之间的借款均是相互独立的，一旦乙方逾期未归还借款本息，甲方中的任何一方均有权单独向乙方追索或者提起诉讼。如乙方逾期支付借款管理费或提供虚假信息的，丁方亦可单独向乙方追索或者提起诉讼。
            <br/><br/>
            ...
            8.3 本协议及其修改或补充均采用通过<%=siteName%>以电子文本形式制成，可以有一份或者多份并且每一份具有同等法律效力，并永久保存在丁方为此设立的专用服务器上备查和保管。双方均认可该形式的协议效力。
            <br/>
            8.4 如果本协议中的任何一条或多条违反适用的法律法规，则该条将被视为无效，但该无效条款并不影响本协议其他条款的效力。 <br/>
            8.5
            本协议签订之日起至借款本息和有关费用全部清偿之日止，甲方或乙方的下列信息如发生变更的，其应当在相关信息发生变更三日内将更新后的信息提供给丁方：本人、本人的家庭联系人及紧急联系人、工作单位、居住地址、住所电话、手机号码、电子邮箱、银行账户的变更。若因任何一方不及时提供上述变更信息而带来的损失或额外费用应由该方自行承担。
            <br/>
        ...
        <table width="80%" class="tab_two">
            ...
            <tr>
                ...
                <td><%=Formater.formatAmount(transfer.amount) %>
                </td>
                <td><%=DateParser.format(transfer.time) %>
                </td>
            </tr>
            <%} %>
        </table>
