...
<%@page import="com.dimeng.p2p.console.servlets.bid.loanmanage.jkgl.AnnexMsk" %>
        ...
        <div class="con clearfix">
            <div class="mt10">
                ...
                <form method="post" enctype="multipart/form-data">
                    ...
                    <%} else { %>
                    <div id="uploader" style="width: 490px; height: 208px; margin-left: 260px; margin-bottom: 103px;">
                        <p>请先选择附件的类型！</p>
                    </div>
                    <%} %>
                </form>
...
<script type="text/javascript">
    $(function () {
        var plupload = function () {
            ...
            $("#uploader").plupload({
                ...
                url: '<%=controller.getURI(request, AddAnnexMsk.class)%>?loanId=<%=loanId%>&typeId=<%=typeId%>&userId=<%=userId%>',
                max_file_size: '4mb',
                chunk_size: '4mb',
                filters: [{
                    title: "Image files",
    ...
    function changeType() {
        var typeId = $("#type").val();
        location.href = "<%=controller.getURI(request, AnnexMsk.class)%>?loanId=<%=loanId%>&userId=<%=userId%>&typeId=" + typeId;
    }

    //附件太长需要滚动条 luoyi 2015-04-28
    function showAttrDialogInfo(msg, type) {
        return '<div class="dialog w510 thickpos" style="margin:-80px 0 0 -255px;">' +
                ...
                '</div>' +
                '<div class="clear"></div>' +
                '<div class="dialog_btn"><button type="button" class="btn3 mr10" onclick="closeInfo()"  class="btn btn001">确认</button></div>' +
                '</div>' +
                '</div>';
    }
