...
function refreshCurrentPage()
{
	...
	arr.push("<div class='popup-box' style='min-height:200px;width:400px'>"
			...
			+"<a class='icon-i popup-close2' href='javascript:void(0);' onclick='refreshPage();'></a>"
			+"</div>"
			+"<div class='popup-content-container pt20 ob20 clearfix'>"
			...
			+"</div>"
			+"<div class='tc f16'>"
			+"<a href='javascript:void(0);' onclick='refreshPage();' class='btn-blue2 btn white radius-6 pl20 pr20' >已完成放款</a>"
			+"<a class='btn btn-blue2 radius-6 pl20 pr20 ml40' href='javascript:void(0);' onclick='refreshPage();'>放款出现问题</a>"
			+"</div>"
			+"</div>"
			+"</div>"
			+"<div class='popup_bg'></div>");
...
function realseSubmit(){
	...
	//window.parent.location.href=urlValue+"&token="+token;
}
