...
<%@page import="com.dimeng.p2p.console.servlets.finance.zjmx.ptzjmx.ExportPtzjDetail" %>
...
<%
    ...
    PagingResult<ZjDetailView> ptzjDetailViewList = (PagingResult<ZjDetailView>) request.getAttribute("ptzjDetailViewList");
    ZjDetailView ptzjDetailCount = (ZjDetailView) request.getAttribute("ptzjDetailCount");
%>
<%
    CURRENT_CATEGORY = "CWGL";
    CURRENT_SUB_CATEGORY = "PTZJMX";
%>
<body>
<div class="right-container">
    <div class="viewFramework-body">
      <div class="viewFramework-content">
                <form id="searchForm" name="form1" action="<%=controller.getURI(request, PtzjDetail.class)%>"
                      method="post">
                    <div class="p20">
                        <div class="border">
                            <div class="title-container"><i class="icon-i w30 h30 va-middle title-left-icon"></i>平台资金明细
                            </div>
                            <div class="content-container pl40 pt30 pr40">
                                <ul class="gray6 input-list-container clearfix">
                                    <li><span class="display-ib mr5">类型明细： </span>
                                        <select name="tradingType" class="border mr20 h32 mw100">
                                            ...
                                            <%
                                                ...
                                                T5122[] t5122s = tradeTypeManage.search(T6110_F06.FZRR, null, null);
                                    ...
                                    <li>
                                        <%if (dimengSession.isAccessableResource(ExportPtzjDetail.class)) {%>
                        ...
                        <div class="border mt20 table-container">
                            <table class="table table-style gray6 tl">
                                ...
                                <tbody class="f12">
                                <%
                                    ZjDetailView[] ptzjDetailViews = null;
                                    if (ptzjDetailViewList != null) {
                                        ptzjDetailViews = ptzjDetailViewList.getItems();
                                    }
                                    if (ptzjDetailViews != null && ptzjDetailViews.length > 0) {
                                        int i = 1;
                                        for (ZjDetailView record : ptzjDetailViews) {
                                            if (record == null) {
                                                continue;
                                            }
                                %>
                                <tr class="tc">
                                    ...
                                    <%if (StringHelper.isEmpty(record.assUserName)) {%>
                                    <td></td>
                                    <%} else {%>
                                    <td><%=record.assUserName%>
                                    </td>
                                    <%}%>
                                    <td><%=record.F09%>
                                    </td>
                        ...
                        <p class="mt5">
                            <span class="mr30">收入总计：<em class="red"><%=Formater.formatAmount(ptzjDetailCount.F06)%>
                            </em> 元</span>
                            <span class="mr30">支出总计：<em class="red"><%=Formater.formatAmount(ptzjDetailCount.F07)%>
                            </em> 元</span>
                        </p>
                        <!--分页-->
                        <%AbstractConsoleServlet.rendPagingResult(out, ptzjDetailViewList); %>
                        <!--分页 end-->
                    </div>
                </form>
            </div>
...
<script type="text/javascript">
    ...
    function showExport() {
        document.getElementById("searchForm").action = "<%=controller.getURI(request, ExportPtzjDetail.class)%>";
        $("#searchForm").submit();
        document.getElementById("searchForm").action = "<%=controller.getURI(request, PtzjDetail.class)%>";
    }
    function onSubmit(){
