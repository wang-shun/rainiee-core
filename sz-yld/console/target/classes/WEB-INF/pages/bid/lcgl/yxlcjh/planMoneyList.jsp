                ...
                <div class="home_main">
                    <form id="form1" action="<%=controller.getURI(request, PlanMoneyList.class)%>" method="post">
                        <div class="box box1 mb15">
                            ...
                            <div class="con">
                                <div class="admin_input admin_pb">
                                    <P>
                                        <span>计划名称：</span>
                                        <input type="text" name="planName" id="textfield" class="yhgl_input"
                                    ...
                                    <p>
                                        ...
                                        <select name="state" class="yhgl_sel">
                                            ...
                                            <option value="DSQ" <%if ("DSQ".equals(request.getParameter("state"))) {%>
                                                    selected="selected" <%}%>>预售中
                                            </option>
                                            <option value="SQZ" <%if ("YFB".equals(request.getParameter("state"))) {%>
                                                    selected="selected" <%}%>>申请中
                                            </option>
                                            <option value="YSX" <%if ("YSX".equals(request.getParameter("state"))) {%>
                                                    selected="selected" <%}%>>已满额
                                            </option>
                                            <option value="YJZ" <%if ("YJZ".equals(request.getParameter("state"))) {%>
                                                    selected="selected" <%}%>>已截止
                                            </option>
                                        </select>
                                    </p>
                                    <div class="clear"></div>
                                    <div class="mt10">
                                        ...
                                        <a href="javascript:void(0)" name="input"
                                           onclick="window.location.href='<%=controller.getURI(request, AddPlanMoney.class)%>'"
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="3" class="yhgl_table tc">
                            ...
                            <%
                                if (planMoneyArray != null) {
                                    for (int i = 0; i < planMoneyArray.length; i++) {
                                        PlanMoney planMoney = planMoneyArray[i];
                                        if (planMoney == null) {
                                            continue;
                                        }
                            %>
                            <tr class="dhsbg tc">
                                ...
                                <td><%=Formater.formatAmount(planMoney.zje)%>
                                </td>
                                <td><%=Formater.formatAmount(planMoney.ktye)%>
                                </td>
                                <td><%=Formater.formatRate(planMoney.earningsRate)%>
                                </td>
                                <td><%=planMoney.joinNumber %>
                                </td>
                                <td><%=TimestampParser.format(planMoney.issueTime) %>
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <span class="blue"><a
                                            href="<%=controller.getURI(request,ViewPlanMoney.class)%>?planMoneyId=<%=planMoney.planMoneyId%>"
                                            class="mr10">查看详情</a></span>
                                    <%} else { %>
