...
<%@page import="com.dimeng.p2p.modules.activity.console.service.entity.JxqCountTotalInfo" %>
<%@page import="com.dimeng.util.ObjectHelper" %>
<%@ page import="com.dimeng.p2p.S63.enums.T6342_F04" %>
<%@ page import="java.math.BigDecimal" %>
<%@ page import="com.dimeng.p2p.console.servlets.spread.hdgl.jxqtj.ExportJxqCount" %>
<html>
<head>
...
<%
    ...
    CURRENT_SUB_CATEGORY = "JXQTJ";
    PagingResult<ActivityCountEntity> result = ObjectHelper.convert(request.getAttribute("result"), PagingResult.class);
    JxqCountTotalInfo info = ObjectHelper.convert(request.getAttribute("info"), JxqCountTotalInfo.class);
    //赠送时间
...
<div class="right-container">
    <div class="viewFramework-body">
      <div class="viewFramework-content">
                <form id="form1" action="<%=controller.getURI(request, SearchJxqCount.class)%>" method="post">
                    <div class="p20">
                        <div class="border">
                            <div class="title-container"><i class="icon-i w30 h30 va-middle title-left-icon"></i>加息券统计
                            </div>
                            <div class="content-container pl40 pt30 pr40">
                                <ul class="gray6 input-list-container clearfix">
                                    <li><span class="display-ib mr5">未使用加息券数量总计</span>
                                        <span class="link-blue"><%=info.unUsedCount%></span>个
                                    </li>
                                    <li class="ml50"><span class="display-ib mr5">已使用加息券数量总计</span>
                                        <span class="link-blue"><%=info.usedCount%></span>个
                                    </li>
                                </ul>
                            </div>
                        </div>
                        <div class="border mt20">
                            <div class="content-container pl40 pt30 pr40">
                                <ul class="gray6 input-list-container clearfix">
                                    ...
                                    <li><span class="display-ib mr5">加息券状态</span>
                                        <select name="status" class="border mr20 h32 mw100">
                                            <%String status = request.getParameter("status"); %>
                                            <option value="">全部</option>
                                            <%for (T6342_F04 s : T6342_F04.values()) {%>
                                            <option value="<%=s.name() %>"
                                    ...
                                    <li>
                                        <%
                                            if (dimengSession.isAccessableResource(ExportJxqCount.class)) {
                                        %>
                        ...
                        <div class="border mt20 table-container">
                            <table class="table table-style gray6 tl">
                                ...
                                <tr class="tc">
                                    ...
                                    <td><%=Formater.formatAmount(ace.interestRate)%>%</td>
                                    <td><%=TimestampParser.format(ace.presentDate, "yyyy-MM-dd HH:mm")%></td>
                                    <td><%=TimestampParser.format(ace.outOfDate, "yyyy-MM-dd")%></td>
                                    <td><%StringHelper.filterHTML(out, ace.activityNum); %></td>
                                    <td><%StringHelper.filterHTML(out, ace.activityName);%></td>
                                    <td><%StringHelper.filterHTML(out, ace.status.getChineseName());%></td>
                                    <td><%=TimestampParser.format(ace.useDate)%></td>
...
<script type="text/javascript">
    ...
    function exportList() {
        var del_url = '<%=controller.getURI(request, ExportJxqCount.class)%>';
        var form = document.forms[0];
        form.action = del_url;
        form.submit();
        form.action = '<%=controller.getURI(request, SearchJxqCount.class)%>';
    }
</script>
</body>
</html>