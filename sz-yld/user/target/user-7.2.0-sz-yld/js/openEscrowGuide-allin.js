...
//发送签约短信
function onSubmitLines(evn){
	
	var openSumbit = $("#openSumbit");
	var realName = $("input[name='realName']");// 姓名
	var mobile = $("input[name='mobile']");//手机号
	var verifyCode = $("input[name='verifyCode']");
	var identificationNo =$("input[name='identificationNo']");//身份证号
	var eUpdateVerify =$("input[name='eUpdateVerify']");
	
	    var bankCode = $("select[name='bankCode']");//银行
	    var bankCard = $("input[name='bankCard']");//卡号
        ...
        if(identificationNo.val().length<=0){
        	showError(identificationNo,"不能为空！");
        	return false;
        }
	    ...
	    if(bankCard.val().length<=0){
			showError(bankCard,"不能为空！");
			return false;
		}
	    if(bankCode == undefined || bankCode.val().length<=0){
			showError(bankCode,"不能为空！");
			return false;
		}
	    var _data = {
                "mobile": mobile.val(),
               // "verifyCode": verifyCode.val(),
                "idCardNo": identificationNo.val(),
	 ...
	 // 发送参数
		$.ajax({
			type:"post",
			url:"/user/capital/fuyouPaySignMsg.htm",
			data:_data,
			dataType: 'text',
		...
		function succFunction(msg) { 
			if (msg == "SUCCESS"){
			   // $("div.popup_bg").show();
			  //  $("#info").html(showDialogInfoReload("签约短信发送成功！", "yes"));
				sendclick(evn);
			} else {
				showError(mobile,msg);
			}
       }  
	}
...
function checkBandPhone(bemil) {
    ...
    var myreg = /^(13|14|15|17|18|19)[0-9]{9}$/;
