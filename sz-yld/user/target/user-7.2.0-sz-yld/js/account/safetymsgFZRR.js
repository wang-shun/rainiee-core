...
function updatePassword(username) {
    ...
    var confPswVal = confPsw.val();
    if (oldPswVal.length <= 0) {
        showError(oldPsw, "不能为空!");
        return false;
    }
    if (newPswVal.length <= 0) {
        showError(newPsw, "不能为空!");
        return false;
    }
    if (newPsw.val() == username) {
    ...
    if (confPswVal.length <= 0) {
        showError(confPsw, "不能为空!");
        return false;
    }
    if (oldPswVal == newPswVal) {
        showError(confPsw, "新密码不能与原密码相同!");
        return false;
    }
    ...
    var data = {
        "old": RSAUtils.encryptedString(key, oldPswVal),
        "new": RSAUtils.encryptedString(key, newPswVal),
        "news": RSAUtils.encryptedString(key, confPswVal)
    };
    $.ajax({
        type: "post",
        dataType: "html",
...
function pupdate(evn) {
    ...
    if (!(flagSfxyyzm != "undefined" && flagSfxyyzm == false)) {
        ...
        if (!myregCode.test(phoneUpdateVerify)) {
            showError(phoneUpdateVerify1, "请输入正确的验证码！");
            return false;
        }
    }
...
$(".w111").click(function () {
    ...
    if (ind == 3) {
        var txp = false;
        if ( isIdCardEmpty || !isIdCardCheck) {
        	txp = true;
        }
        if (txp) {
        ...
        if(chargeMustWithdrawPsd && isTxPasswordEmpty){
        	txp = true;
        }
        if (txp) {
...
$(function () {
    $(".coden").click(function () {
        ...
        /**if(type=="newtwo"){
				...
				var oldEmail = '<%=realEmail%>';
				
				var emilFlgCode = $("input[name='emilFlg']").val();
				if(emilFlgCode!=2){
					return false;
				}
				/* var myreg = /^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,4}$/; */
				...
				var data={"evencheck":"emil","utp":"update","ctp":"new","value":emil.val(),"code":code.val(),"oldVal":oldEmail};
				$.ajax({
					type:"post",
					dataType:"html",
					...
					success:function(data){
						...
						if(data == "01"){
							...
							$("#e2").hide();
							$("#e3").show();
							$("#e3").find("p").eq(1).html(emil.val().substring(0,1)+"*****"+emil.val().split("@")[1]);
							reload();
						}
					}
				});
        ...
        /**if(type=="epnewtwo"){
				...
				var data={"evencheck":"emil","utp":"phoneemil","ctp":"pnew","value":emil.val(),"code":code.val(),"oldVal":oldVal};
				$.ajax({
					type:"post",
					dataType:"html",
					url:"<%=controller.getURI(request, CheckNewCode.class)%>",
					...
					success:function(data){
						...
						if(data == "01"){
							...
							$("#ep2").hide();
							$("#ep3").show();
							$("#ep3").find("p").eq(1).html(emil.val().substring(0,1)+"*****"+emil.val().split("@")[1]);
							reload();
						}
					}
				});
        ...
        if (type == "ipoldone") {
            ...
            if(isTxPasswordEmpty){
	            if (txpassword.length > 0 && txpassword.val().length <= 0) {
        ...
        if (type == "pswtwo") {
            ...
            if (emil.val().length <= 0) {
                ...
                $error.show();
                return false;
            }else if (emil.val()=="<%=data.username%>") {
                $error = emil.nextAll("p[errortip]");
                $tip = emil.nextAll("p[tip]");
                $error.addClass("error_tip");
...
function isIdCardNo(num) {
    ...
    if (!(/(^\d{15}$)|(^\d{17}([0-9]|X)$)/.test(num))) {
        alert('输入的身份证号不符合规定！15位号码应全为数字，18位号码末位可以为数字或X。');
        return false;
...
$(function () {
    ...
    $("#safeSubmit").click(function () {
        ...
        $.each($("div#box8 .question input[type='hidden']"), function (i, o) {
           ...
           // var type = $(o).attr("id").substr("question".length);
            ...
            if(!questionId){
                $(o).parent().parent().find("p[errortip]").addClass("error_tip").html("请选择密保问题！").show();
                checkFlag = false;
                return false;
            }
            var val = $(o).parent().parent().find("input[data=hide]").val();
            if (!val) {
                $(o).parent().parent().find("p[errortip]").addClass("error_tip").html("不能为空!").show();
            ...
            if (re.test(val)) {
                $(o).parent().parent().find("p[errortip]").addClass("error_tip").html("存在特殊字符").show();
                $(o).parent().parent().find("input[data=hide]").focus();
                checkFlag = false;
                return false;
            }
        ...
        if (!checkFlag) return;
        var tokenValue = $("input[name='"+tokenName+"']").val();
        var data = {"tokenKey": tokenValue, "param": param.substr(0, param.length - 1)};
        $.ajax({
            type: "post",
            dataType: "html",
   ...
   /* var obj1 = $("select[name='question1']");
	var obj2 = $("select[name='question2']");
	var obj3 = $("select[name='question3']");
    ...
    /*var obj4 = $("select[name='question4']");
    var obj5 = $("select[name='question5']");
    var obj6 = $("select[name='question6']");
    ...
    /*var obj7 = $("select[name='question7']");
    var obj8 = $("select[name='question8']");
    var obj9 = $("select[name='question9']");
...
function applyGuarantor(){
    ...
    else if (isTxPasswordEmpty && chargeMustWithdrawPsd==true ){
