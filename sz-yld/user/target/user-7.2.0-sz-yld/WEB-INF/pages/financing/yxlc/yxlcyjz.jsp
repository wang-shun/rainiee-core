                            ...
                            <div class="no_table user_bolr bot">
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="user_table tc">
                                    ...
                                    <tr>
                                        <td>
                                            <a class="blue"
                                               href="<%configureProvider.format(out, URLVariable.FINANCING_YX_XQ); %><%=ebf.planId + rewriter.getViewSuffix()%>">
                                                <%StringHelper.filterHTML(out, ebf.name);%>
                                            </a>
                                        </td>
                                        <td><%=Formater.formatAmount(ebf.addMoney) %>元</td>
                                        <td><%=Formater.formatRate(ebf.rate) %>
                                        </td>
                                        <td><%=Formater.formatAmount(ebf.money) %>元</td>
                                        <td><%=Formater.formatAmount(ebf.takeMoney) %>元</td>
                                        <td><%=Formater.formatDate(ebf.endTime) %>
                                        </td>
                                        <td><a target="_blank"
                                               href="<%=controller.getViewURI(request, com.dimeng.p2p.user.servlets.financing.agreement.Yxlc.class) %>?id=<%=ebf.planId %>">合同</a>
                                        </td>
                                    </tr>
                                    <%
                                            }
