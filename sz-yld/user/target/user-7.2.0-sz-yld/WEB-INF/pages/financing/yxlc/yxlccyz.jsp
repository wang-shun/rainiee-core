                        ...
                        <div class="Menubox">
                            <ul>
                                ...
                                <li style="border-right:1px #d7dfe3 solid;"><a
                                        href="<%=controller.getViewURI(request, Yxlcyjz.class) %>">已截止的优选计划</a></li>
                            </ul>
                        </div>
                    </div>
                            ...
                            <div class="no_table user_bolr bot">
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="user_table tc">
                                    ...
                                    <tr>
                                        <td>
                                            <a class="blue"
                                               href="<%configureProvider.format(out, URLVariable.FINANCING_YX_XQ); %><%=ibf.planId + rewriter.getViewSuffix()%>">
                                                <%StringHelper.filterHTML(out, ibf.name); %>
                                            </a>
                                        </td>
                                        <td><%=Formater.formatAmount(ibf.addMoney) %>元</td>
                                        <td><%=Formater.formatRate(ibf.rate) %>
                                        </td>
                                        ...
                                        <td><%=DateParser.format(ibf.nextDay) %>
                                        </td>
                                        <td><%if (ibf.status == PlanState.YSX) { %>回款中<%} else { %>申请中<%} %></td>
                                        <td><a target="_blank"
                                               href="<%=controller.getViewURI(request, com.dimeng.p2p.user.servlets.financing.agreement.Yxlc.class) %>?id=<%=ibf.planId %>">合同</a>
                                        </td>
                                    </tr>
                                    <%
                                            }
