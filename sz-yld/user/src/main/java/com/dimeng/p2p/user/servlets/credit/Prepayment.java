...
public class Prepayment extends AbstractCreditServlet
{
        ...
        throws Throwable
    {
        ...
        int[] orderIds = manage.prepayment(id, number);
        try
        {
            ...
            if (orderIds != null && orderIds.length > 0)
            {
                ...
                manage.writeFrontLog(FrontLogType.TQHK.getName(), "前台用户提前还款");
            }
        }
        catch (Throwable e)
        {
        ...
        String url = configureProvider.format(URLVariable.PAY_PERPAYMENT_URL_SECOND);
        getController().prompt(request, response, PromptLevel.INFO, "恭喜您，提前还款成功！");
        //sendRedirect(request, response, getController().getViewURI(request, Repaying.class));
