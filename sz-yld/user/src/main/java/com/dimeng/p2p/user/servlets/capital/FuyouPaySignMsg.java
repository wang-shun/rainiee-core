...
public class FuyouPaySignMsg extends AbstractFuyoupayServlet 
{
  ...
  /*      if (t6110.F06 == T6110_F06.FZRR)
        {
            processRequest(request, response, "自然人用户才能进行协议签约");
        }*/
        //用户编号
        ...
        //银行卡号
        String bankCard = request.getParameter("bankCard").replaceAll("\\s", "");
        if (StringHelper.isEmpty(bankCode) || StringHelper.isEmpty(bankCard))
        {
            processRequest(request, response, "银行卡信息不能为空");
            return;
        }   
        //身份证号码
...
/*       if (!StringHelper.isEmpty(checkMsg))
       ...
       if (safetyManage.isIdcard(idCardNo, T6110_F06.ZRR))
        {
            processRequest(request, response, "该身份证已认证过");
            return;
        }
       int number = Integer.parseInt(configureProvider.getProperty(SystemVariable.SM_RZCS));
        ...
        {
            processRequest(request, response, "您今天实名认证太频繁，请明天再试");
            return;
        }
        ...
        {
            processRequest(request, response, "当前银行卡号已存在");
            return;
        }
        // 添加用户信息
        ...
        if (!safetyManage.isPhone(mobile,accountId))
        {
       ...
       if ("0000".equals(signMsgResult.getRESPONSECODE()))
        ...
        else
        {
            ...
            processRequest(request, response, signMsgResult.getRESPONSEMSG());
        }
    }
        ...
        throws Throwable
    {
        ...
        BankCardQuery query = new BankCardQuery()
        {
            ...
            {
                return request.getParameter("bankCard").replaceAll("\\s", "");
            }
            
            @Override
            public int getBankId()
            {
                return IntegerParser.parse(request.getParameter("bankCode"));
            }
