...
public class ZXingCodeUtil {
     ...
     */
    private void addQRCodeLogo(BufferedImage bim, File logoPic, LogoConfig logoConfig) throws Exception {
        try {
            // 对象流传输
     ...
     * @throws Exception 异常上抛
     */
    public BufferedImage getQRCodeBufferedImage(ZXingConfig zxingconfig) throws Exception {
        // Google配置文件
        ...
        try {
            ...
            if (zxingconfig.isLogoFlg()) {
                this.addQRCodeLogo(image, new File(zxingconfig.getLogoPath()), zxingconfig.getLogoConfig());
            }
    ...
    public static void main(String[] args) throws WriterException {
        String content = "http://61.145.159.156:8410/register.html?code=ytrt1w";
        ...
        try {
            ...
            zxingconfig.setHeight(160);									// 设置二维码高度
            zxingconfig.setWidth(160);									// 设置二维码宽度
            zxingconfig.setLogoPath("D:/logo.jpg"); 					// 设置Logo图片
            zxingconfig.setLogoConfig(new LogoConfig());    			// Logo图片参数设置   
            zxingconfig.setLogoFlg(false);   							// 设置生成Logo图片
            BufferedImage bim = zp.getQRCodeBufferedImage(zxingconfig);	// 生成二维码
