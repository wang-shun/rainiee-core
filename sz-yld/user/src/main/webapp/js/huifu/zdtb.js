$(function() {
	...
	$("#zztb").click(function() {
		...
		if(count>=3){
			...
			$("#info").html(showForwardInfo("您已达到新增自动投资规则数量最大值，<br/>请删除已有规则，再重新新增规则.","doubt","javascript:closeDiv();"));
		}else{
			$("#errorjk").hide();
			$("p[errortip]").hide();
			$("p[tip]").show();
...
function update(status,id){
	...
	if('QY' == status) {
		...
		url = "/user/huifu/hFZdtbClose.htm";
	}
...
function deleteZdtb(id, status){
	if('QY' == status) {
		$(".popup_bg").show();
		$("#info").html(showDialogInfo("自动投标开启中，不能进行删除","error"));
	} else {
		$(".popup_bg").show();
