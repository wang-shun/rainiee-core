...
<div class="contain clearfix">
    ...
    <div class="contain_main clearfix ">
        <div class="plan_tab_con01 clearfix">
            ...
            <%
                if (t6212s != null && t6212s.length > 0) {
                    for (T6212 t6212 : t6212s) {
            %>

            <div class="pub_title pt10"><%=t6212.F02 %>
            </div>
            <div class="pt10">
                <div class="highslide-gallery gyb_wh">
                    <%
                        if (isResult && t6233s != null && t6233s.length > 0) {
                            for (T6233 attachment : t6233s) {
                                if (attachment == null || attachment.F03 != t6212.F01) {
                                    continue;
                                }%>
                    <a href="<%=controller.getURI(request, Annex.class)%>?id=<%=attachment.F01 %>" class="highslide"
                       onclick="return hs.expand(this)">
                        <img src="<%=controller.getURI(request, Annex.class)%>?id=<%=attachment.F01 %>"
                             alt="Highslide JS" width="185" height="116"/>

                        <p class="formTitle-21"><%StringHelper.filterHTML(out, attachment.F04);%></p>
                    </a>
                    <%
                        }
                    } else if (t6232s != null && t6232s.length > 0) {
                        for (T6232 attachment : t6232s) {
                            if (attachment == null || attachment.F03 != t6212.F01) {
                                continue;
                            }
                    %>
                    <a href="<%=fileStore.getURL(attachment.F04) %>" class="highslide" onclick="return hs.expand(this)">
                        <img src="<%=fileStore.getURL(attachment.F04) %>" alt="Highslide JS" width="185" height="116"/>

                        <p class="formTitle-21"><%StringHelper.filterHTML(out, attachment.F05);%></p>
                    </a>
                    <%
                            }
                        }
