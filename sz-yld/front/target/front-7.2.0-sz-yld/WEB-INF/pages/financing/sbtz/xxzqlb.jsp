...
<head>
    ...
    <title>线下债权转让-<%=configureProvider.getProperty(SystemVariable.SITE_TITLE)%>
    </title>
...
<div class="contain clearfix">
    <div class="w1002 clearfix">
        <div>
            <div class="item_list">
                <div class="plan_tab clearfix">
                    <ul>
                        ...
                        <li>
                            <a href="<%=controller.getViewURI(request, com.dimeng.p2p.front.servlets.financing.sbtz.Index.class)%>">企业标</a>
                        </li>
                        <li>
                            <a href="<%=controller.getViewURI(request, com.dimeng.p2p.front.servlets.financing.sbtz.Grlb.class)%>">个人标</a>
                        </li>
                        <li class="hover">线下债权转让</li>
                        <li>
                            <a href="<%=controller.getViewURI(request, com.dimeng.p2p.front.servlets.financing.sbtz.Zqzrlb.class)%>">线上债权转让</a>
                        </li>
                ...
                <div class="bd jtab-con">
                    <div class="screening">
                        ...
                        <div class="con">
                            ...
                            <dl class="filtrate">
                                ...
                                <dd>
                                    <%
                                        if (parameter.contains(PROGRESS_KEY)) {
                                            parameter.remove(PROGRESS_KEY);
                                    %>
                                    <a href="<%=parameter.toString()%>">不限</a>
                                    <%} else {%>
                                    <a class="cur" href="<%=parameter.toString()%>">不限</a>
                                    <%}%>
                                    <%
                                        if (parameter.contains(PROGRESS_KEY, "1")) {
                                            parameter.remove(PROGRESS_KEY, "1");
                                    %>
                                    <a class="cur" href="<%=parameter.toString()%>">50%以下</a>
                                    <%
                                    } else {
                                        parameter.set(PROGRESS_KEY, "1");
                                    %>
                                    <a href="<%=parameter.toString()%>">50%以下</a>
                                    <%}%>
                                    <%
                                        if (parameter.contains(PROGRESS_KEY, "2")) {
                                            parameter.remove(PROGRESS_KEY, "2");
                                    %>
                                    <a class="cur" href="<%=parameter.toString()%>">50%-80%</a>
                                    <%
                                    } else {
                                        parameter.set(PROGRESS_KEY, "2");
                                    %>
                                    <a href="<%=parameter.toString()%>">50%-80%</a>
                                    <%}%>
                                    <%
                                        if (parameter.contains(PROGRESS_KEY, "3")) {
                                            parameter.remove(PROGRESS_KEY, "3");
                                    %>
                                    <a class="cur" href="<%=parameter.toString()%>">80%以上</a>
                                    <%
                                    } else {
                                        parameter.set(PROGRESS_KEY, "3");
                                    %>
                            ...
                            <dl class="filtrate">
                                ...
                                <dd>
                                    ...
                                    <%}%>
                                    <%
                                        if (parameter.contains(TYPE_KEY, "TBZ")) {
                                            parameter.remove(TYPE_KEY, "TBZ");
                                    %>
                                    <a href="<%=parameter.toString()%>" class="cur">可投资</a>
                                    <%
                                    } else {
                                        parameter.set(TYPE_KEY, "TBZ");
                                    %>
                                    <a href="<%=parameter.toString()%>">可投资</a>
                                    <%}%>
                                    <%
                                        if (parameter.contains(TYPE_KEY, "HKZ")) {
                                            parameter.remove(TYPE_KEY, "HKZ");
                                    %>
                                    <a href="<%=parameter.toString()%>" class="cur">还款中</a>
                                    <%
                                    } else {
                                        parameter.set(TYPE_KEY, "HKZ");
                                    %>
                                    <a href="<%=parameter.toString()%>">还款中</a>
                                    <%}%>
                                    <%
                                        if (parameter.contains(TYPE_KEY, "YJQ")) {
                                            parameter.remove(TYPE_KEY, "YJQ");
                                    %>
                                    <a href="<%=parameter.toString()%>" class="cur">已还款</a>
                                    <%
                                    } else {
                                        parameter.set(TYPE_KEY, "YJQ");
                                    %>
                   ...
                   <%if (total.totleMoney.doubleValue() >= 100000000) {%>
		        	   <em class="f24"><%=Formater.formatAmount(total.totleMoney.doubleValue() / 100000000)%>
                       </em>亿元
		        	<%} else if (total.totleMoney.doubleValue() >= 10000 && total.totleMoney.doubleValue() < 100000000) {%>
		        		<em class="f24"><%=Formater.formatAmount(total.totleMoney.doubleValue() / 10000)%>
                        </em>万元
		        	<%} else {%>
		        		<em class="f24"><%=Formater.formatAmount(total.totleMoney)%>
                        </em>元
		        	<%}%>
                   ...
                   <%
		        	   ...
		        	   <em class="f24"><%=Formater.formatAmount(total.userEarnMoney.doubleValue() / 100000000)%>
                       </em>亿元
		        	<%} else if (total.userEarnMoney.doubleValue() >= 10000 && total.userEarnMoney.doubleValue() < 100000000) {%>
		        		<em class="f24"><%=Formater.formatAmount(total.userEarnMoney.doubleValue() / 10000)%>
                        </em>万元
		        	<%} else {%>
		        		<em class="f24"><%=Formater.formatAmount(total.userEarnMoney)%>
                        </em>元
		        	<%}%>
                   </span></li>
                                </ul>
            ...
            %>
            </span><span class="w200"><a title="<%=t6240.F05%>"
                                         href="<%=controller.getPagingItemURI(request, Bdxq.class,creditInfo.F02)%>">
                                                    ...
                                                    <td><%
                                                        if (creditInfo.F06.doubleValue() >= 10000) {%><span
                                                            class="f16"><%=creditInfo.F06.doubleValue() / 10000%></span>
                                                        <span class="f12">万元</span>
                                                        <%} else {%><span
                                                                class="f16"><%=Formater.formatAmount(creditInfo.F06)%></span><span
                                                                class="f12">元</span><%}%>
                                                    </td>
                                                    <td><span
                                                            class="f16"><%=Formater.formatRate(creditInfo.F07, false)%></span><span
                                                            class="f12">%</span></td>
                                                    ...
                                                    <td><%
                                                        if (creditInfo.F08.doubleValue() >= 10000) {%><span
                                                            class="f16"><%=creditInfo.F08.doubleValue() / 10000%></span>
                                                        <span class="f12">万元</span>
                                                        <%} else {%><span
                                                                class="f16"><%=Formater.formatAmount(creditInfo.F08)%></span><span
                                                                class="f12">元</span><%}%></td>
                                                    <td>
                                                        <%if (creditInfo.F11 != T6230_F20.YFB) {%>
                                                        <div class="pl30"><span class="ui-list-field w110">
	            <strong class="ui-progressbar-mid ui-progressbar-mid-<%=(int)(creditInfo.proess*100)%>"><em><%=Formater.formatProgress(creditInfo.proess)%>
                </em></strong></span>
                                                        </div>
                                                        <%} else { %>
                                                    ...
                                                    <td><%if (creditInfo.F11 == T6230_F20.TBZ) {%>
                                                        ...
                                                        <%} else {%>
                                                        <%if (creditInfo.F11 == T6230_F20.HKZ) {%><span
                                                                class="btn btn02 ml15">还款中</span><%} else {%><span
                                                                class="btn btn06 ml10"><%=creditInfo.F11.getChineseName() %></span><%}%><%}%>
                                                    </td>
                                                </tr>
                                                <%
                                                        }
