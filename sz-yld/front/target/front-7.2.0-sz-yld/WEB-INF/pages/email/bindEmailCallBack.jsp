...
<div class="main_bg" style="padding-top:20px;">
    <div class="login_mod">
        <div class="login_hd"><i class="icon"></i><span class="gray3 f18">绑定邮箱</span></div>
        <%if ("success".equals(status)) { %>
        <div class="tip_info mt50 pt50">
            <div class="successful"></div>
            <div class="tips">
                <span class="f18 gray3">绑定邮箱成功！</span>
            </div>
        </div>
        ...
        <%} else { %>
        <div class="tc mt50"><a href="<%=controller.getStaticPath(request)%>/user/login.html" class="btn06">返回登录页面</a>
        </div>
        <%} %>
        <%} else { %>
        <div class="tip_info mt50 pt50">
            <div class="error"></div>
            <div class="tips">
                <span class="f18 gray3">绑定邮箱失败！</span><br/><%=StringHelper.isEmpty(message) ? "" : message%>
            </div>
        </div>
        <%if (dimengSession != null && dimengSession.isAuthenticated()) { %>
