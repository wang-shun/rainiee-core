$(function(){
	...
	function checkBid(){
		...
		if(gmMoney == 0 || gmMoney.length==0 || !(parseInt(gmMoney)%parseInt(minMoney)===0)){
			$("#info").html(showDialogInfo("你的投资金额必须为最低投资额的整数倍","perfect"));
			$("div.popup_bg").show();
			return;
		}
		
		if(parseInt(maxMoney) < parseInt(gmMoney) + parseInt(tzMoney)){
			$("#info").html(showDialogInfo("你的投资已经超出计划的个人投资最大限额","perfect"));
			$("div.popup_bg").show();
			return;
		}
		if(parseInt(gmMoney) < parseInt(minMoney)){
			$("#info").html(showDialogInfo("你的投资金额低于计划的最低投资金额","perfect"));
			$("div.popup_bg").show();
			return;
		}
		if(parseInt(syMoney) < parseInt(gmMoney)){
			$("#info").html(showDialogInfo("你的投资金额大余计划的剩余投资金额","perfect"));
			$("div.popup_bg").show();
			return;
		}
		if(parseInt(kyMoney) < parseInt(gmMoney) + parseInt(gmMoney*jrfl)){
			var url=$("#charge").val();
			$("#info").html(showForwardInfo('你的账户余额不足进行本次投资 ，请充值，点击"确定"，跳转充值页面，点击"取消"返回当前页面',"perfect",url));
			$("div.popup_bg").show();
			return;
		}
