...
<head>
    ...
    <title>用户信息-<%=configureProvider.getProperty(SystemVariable.SITE_TITLE)%>
    </title>
...
<div class="contain clearfix">
    <div class="w1002 top_title">
        <span class="fl"><a
                href="<%=controller.getViewURI(request, com.dimeng.p2p.front.servlets.Index.class) %>">首页</a> &gt;  用户信息</span>
    </div>
    <%
            ...
            <table width="100%" border="0" align="left" cellspacing="0">
                <tr>
                    <td width="188" valign="top" class="w150 line">
                        <span class="pic"><img
                                src="<%if(StringHelper.isEmpty(userInfo.F15)){%><%=controller.getStaticPath(request)%>/images/tx.jpg<%}else{%><%=fileStore.getURL(userInfo.F15) %><%}%>"
                                width="127" height="125" class="pic_img"/></span>
                    </td>
                    <td width="24%" valign="top" class="line">
                        <div class="tit">理财统计</div>
                        <div class="clearfix"><span class="fl w230">持有债权数量（笔）</span><span
                                class="fl tl"><%=userInfo.cyzqsl %></span></div>
                        <div class="clearfix"><span class="fl w230">持有理财计划数量（笔）</span><span
                                class="fl tl"><%=userInfo.cylcjhsl %></span></div>
                    </td>
                    <td valign="top">
                        <div class="tit">借款统计</div>
                        <div class="clearfix">
                            <div class="mr20 w220 fl"><span class="fl w150">发布借款数量（笔）</span><span
                                    class="fl tl"><%=userInfo.fbjksl %></span></div>
                            <div class="mr20 w220 fl"><span class="fl w150">逾期次数（次）</span><span
                                    class="fl tl"><%=userInfo.yqcs %></span></div>
                        </div>
                        <div class="clearfix">
                            <div class="mr20 w220 fl"><span class="fl w150">成功借款数量（笔）</span><span
                                    class="fl tl"><%=userInfo.cgjksl %></span></div>
                            <div class="mr20 w220 fl"><span class="fl w150">逾期金额（元）</span><span
                                    class="fl tl"><%=Formater.formatAmount(userInfo.yqje)%></span></div>
                        </div>
                        <div class="clearfix">
                            <div class="mr20 w220 fl"><span class="fl w150">未还清借款数量（笔）</span><span
                                    class="fl tl"><%=userInfo.whqjksl %></span></div>
                            <div class="mr20 w220 fl"><span class="fl w150">严重逾期笔数（笔）</span><span
                                    class="fl tl"><%=userInfo.yzyqcs %></span></div>
                        </div>
                    </td>
                </tr>
            </table>
            ...
            <div class="info">
                <table width="100%" cellspacing="0">
                    <tr class="big_tit">
                        ...
                        <td width="98" class="w100">年化利率（%）</td>
	            ...
	            <%} else {%>信<%}%>
                                         ...
                                         href="<%=controller.getPagingItemURI(request, com.dimeng.p2p.front.servlets.financing.sbtz.Bdxq.class,creditInfo.F02)%>"><%
                                StringHelper.filterHTML(out, StringHelper.truncation(creditInfo.F04, 10));%></a></span>
                            </div>
                        </td>
                        <td><%=Formater.formatRate(creditInfo.F07)%>
                        </td>
                        <td>
                            <%if (creditInfo.F06.doubleValue() >= 10000) {%>
                            <span class="f16"><%=Formater.formatAmount(creditInfo.F06.doubleValue() / 10000)%></span><span
                                class="f12">万元</span>
                            <%} else {%>
                            <span class="f16"><%=Formater.formatAmount(creditInfo.F06)%><span
                                    class="f12">元</span></span>
                            <%}%>
                        </td>
