...
<head>
    ...
    <title>黑名单-<%=configureProvider.getProperty(SystemVariable.SITE_TITLE)%>
    </title>
    ...
    <div class="contain_main clearfix">
        <div class="blacklist">
            <table width="100%" border="0" align="left" cellspacing="0">
                ...
                <%
                    PagingResult<UserInfo> result = service.search(new Paging() {
                    ...
                    UserInfo[] userBlacklist = result.getItems();
                    if (userBlacklist != null) {
                        for (UserInfo userBlack : userBlacklist) {
                            if (userBlack == null) {
                                continue;
                            }
                ...
                <tr>
                    <td valign="top" class="w150">
                        <span class="pic"><img
                                src="<%if(StringHelper.isEmpty(userBlack.F15)){%><%=controller.getStaticPath(request)%>/images/tx.jpg<%}else{%><%=fileStore.getURL(userBlack.F15) %><%}%>"
                    ...
                    <td valign="top">逾期次数（次）：<%=entity.F02 %>次<br/>
                        ...
                        待还金额（元）：<%=service.getDhje(userBlack.F01) %>元
                    </td>
                </tr>
                <%
                        }
