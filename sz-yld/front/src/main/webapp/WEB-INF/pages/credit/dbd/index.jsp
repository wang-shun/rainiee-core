...
<head>
    ...
    <meta name="keywords" content="个人担保贷款，个人担保贷款申请，小额担保贷款"/>
    <meta name="description" content="<%=configureProvider.getProperty(SystemVariable.SITE_NAME)%>为您提供最新的个人担保贷款产品,在线快速申请,<%=configureProvider.getProperty(SystemVariable.SITE_NAME)%>免费为你审核评估,专业团队为您量身定制信用贷款方案,额度高，利息低，流程简单！"/>
    <meta http-equiv="X-UA-Compatible" content="IE=EmulateIE8"/>
    <meta http-equiv="Pragma" CONTENT="no-cache"/>
    <meta http-equiv="Cache-Control" CONTENT="no-cache"/>
    <meta http-equiv="Expires" CONTENT="0"/>
    <title>担保贷申请_<%=configureProvider.getProperty(SystemVariable.SITE_TITLE)%></title>
...
<div class="con-container">
	<h3><span class="title"><%=configureProvider.getProperty(SystemVariable.SITE_NAME)%>担保贷</span></h3>
	<%configureProvider.format(out,HTMLVariable.LOAN_PRODUCTS_DBD); %>
    <div class="tr">
    	<%
             if (dimengSession == null || !dimengSession.isAuthenticated()) {
         %>
         <a href="<%=controller.getViewURI(request,com.dimeng.p2p.front.servlets.credit.dbd.Dbd.class)%>"
            class="btn">立即申请</a>
         <%
         } else {
...
<!--申请条件-->
    ...
    <div class="title-pic"><img src="<%=controller.getStaticPath(request)%>/images/dbd-icon-1.png"></div>
     ...
     <div>
     <%configureProvider.format(out,HTMLVariable.LOAN_APPLY_CONDITION_DBD); %>
     </div>
    </div>
   <!--申请条件 end-->
   ...
   <!--借款方式-->
    ...
    <div class="title-pic"><img src="<%=controller.getStaticPath(request)%>/images/dbd-icon-2.png"></div>
     ...
     <ul class="f16">
        ...
        <p><%=Formater.formatAmount(DoubleParser.parse(configureProvider.getProperty(SystemVariable.GUARANTEE_LOAN_AMOUNT_MIN))) %>-
                        <%=Formater.formatAmount(DoubleParser.parse(configureProvider.getProperty(SystemVariable.GUARANTEE_LOAN_AMOUNT_MAX))) %>元</p>
        </li>
        <li class="mb20"><h3 class="bold">借款年化利率</h3>
        <p><%=Formater.formatRate(DoubleParser.parse(configureProvider.getProperty(SystemVariable.GUARANTEE_LOAN_RATE_MIN)))%>
           -
           <%=Formater.formatRate(DoubleParser.parse(configureProvider.getProperty(SystemVariable.GUARANTEE_LOAN_RATE_MAX)))%>
        </p>
        </li>
        
        <li class="mb20"><h3 class="bold">借款期限</h3>
        <p><%configureProvider.format(out,SystemVariable.GUARANTEE_LOAN_QX_MIN); %>-<%configureProvider.format(out,SystemVariable.GUARANTEE_LOAN_QX_MAX); %>个月</p>
        </li>
        
        <li class="mb20"><h3 class="bold">审核时间</h3>
        <p><%configureProvider.format(out,SystemVariable.GUARANTEE_LOAN_SHSJ_MIN); %>-<%configureProvider.format(out,SystemVariable.GUARANTEE_LOAN_SHSJ_MAX); %>个工作日</p>
        </li>
     </ul>
     </div>
   ...
   <ul > 
   <%if(Boolean.parseBoolean(configureProvider.getProperty(SystemVariable.GUARANTEE_IS_SHOW_DEBX))){ %>
    <li>◆ 等额本息；</li> 
    <%} %>
    <%if(Boolean.parseBoolean(configureProvider.getProperty(SystemVariable.GUARANTEE_IS_SHOW_MYFX))){ %>
    <li>◆ 每月付息,到期还本；</li>
    <%} %>
    <%if(Boolean.parseBoolean(configureProvider.getProperty(SystemVariable.GUARANTEE_IS_SHOW_YCFQ))){ %>
    <li>◆ 本息到期一次付清；</li>
    <%} %>
    <%if(Boolean.parseBoolean(configureProvider.getProperty(SystemVariable.GUARANTEE_IS_SHOW_DEBJ))){ %>
    <li>◆ 等额本金；</li>
    <%} %> 
    </ul>
     </div>
