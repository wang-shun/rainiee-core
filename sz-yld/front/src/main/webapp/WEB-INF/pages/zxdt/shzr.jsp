...
<head>
    ...
    <title><%=articleManage.getCategoryNameByCode("SHZR") %>  最新动态</title>
        ...
        <div class="safety_list">
            <ul>
                <%
                    ...
                    PagingResult<T5011> results = articleManage.search(T5011_F02.SHZR, new Paging() {
                ...
                %>
                <li>
                    <div class="pic"><%if (!StringHelper.isEmpty(article.F09)) {%><img
                            src="<%=controller.getStaticPath(request)%><%=fileStore.getURL(article.F09)%>"/><%}%></div>
                    <div class="name"><a target="_blank"
                                         href="<%=controller.getPagingItemURI(request, Shzr.class,article.F01 )%>"><%
                        StringHelper.filterHTML(out, article.F06); %></a></div>
                    ...
                    <div class="details"><a target="_blank"
                                            href="<%=controller.getPagingItemURI(request, Shzr.class,article.F01 )%>">查看详情</a>
                    </div>
                </li>
                <%
                        }
