...
public class Xjd extends AbstractCreditServlet
{
        ...
        throws Throwable
    {
        ...
        if (isMustEmail && !mge.isEmail())
        {
            SafetymsgViewManage safeManage = serviceSession.getService(SafetymsgViewManage.class);
            String info =
                "申请信用贷必须必须认证邮箱，请您到<a id='to_validate' href=\""
        ...
        {
            getController().prompt(request, response, PromptLevel.ERROR, "非自然人账户不能申请信用贷");
            getController().sendRedirect(request, response, getController().getViewURI(request, Index.class));
            return;
        }
        ...
        if (!isNetSigned)
        {
            getController().prompt(request,
                response,
                PromptLevel.ERROR,
                "申请信用贷必须先网签合同认证，<br/>请您到<a id='to_validate' href=\""
        ...
        if (!flag)
        {
            ...
            forwardView(request, response, Xjd.class);
            return;
        }
        //信用贷
        t6231.F35 = T6231_F35.XYD;
        manage.add(t6230, t6231, null);
