...
public class FinancialPurchase extends AbstractPayServlet {
	...
	protected void processPost(HttpServletRequest request,
        ...
        FinancialPurchaseManage manage = serviceSession.getService(FinancialPurchaseManage.class);
		int orderId = manage.addOrder(yxId, amount);
        FinancialPurchaseExecutor executor = getResourceProvider().getResource(FinancialPurchaseExecutor.class);
		executor.submit(orderId, null);
		executor.confirm(orderId, null);
		getController().prompt(request, response, PromptLevel.INFO, "恭喜你，投资成功。");
